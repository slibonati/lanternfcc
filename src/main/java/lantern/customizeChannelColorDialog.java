...
//	pane.add(field);
		...
		button.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent event) {
				...
				try {
					...
					if (newColor != null) {
						sharedVariables.channelOn[num1] = 1; // channelOn accepts numbers up to 500 and indicates it is
																// colorized, not a default color
						sharedVariables.channelColor[num1] = newColor; // channelColor accepts channel numbers up to 500
																		// and gives their color. you wouldnt need to
																		// look at this unless channelOn for that
																		// channel number was set to 1
		...
		/*
		 * JButton done = new JButton("Done"); done.addActionListener(new
		 * ActionListener() { public void actionPerformed(ActionEvent event) {
		 * dispose(); }}); pane.add(button);
		 * 
		 * pane.setLayout(new GridLayout(5,1)); pane.add(greeting); pane.add(field);
