...
//  essentially it switches the game that is shown on the  board
	public void makehappen(int i) {
		...
		int lastTabNumber = -1; // for erasing tabi'monbrackground from last tab make it active tab color since
								// it was just active
		...
		if (sharedVariables.mygame[gameData.LookingAt] != null
		...
		try {
			prefixHandler.setSelectedIndex(comboMemory[i]);
		} catch (Exception dummy) {
		}
		sharedVariables.lastButton = gameData.BoardIndex;

		try {
			sharedVariables.moveSliders[gameData.BoardIndex]
					.setMaximum(sharedVariables.mygame[gameData.LookingAt].turn);
		...
		if (gameData.LookingAt == sharedVariables.engineBoard && sharedVariables.engineOn == true) {
			...
			if (gameData.LookingAt == oldLookingAt && tryOnce == true) {
				...
				try {
					...
					sharedVariables.mygamedocs[gameData.LookingAt]
							.insertString(sharedVariables.mygamedocs[gameData.LookingAt].getLength(), promptText, null);
				} catch (Exception ee) {
				}
					...
					public void run() {
						try {
							sharedVariables.gametable[gameData.BoardIndex]
									.setModel(sharedVariables.mygametable[gameData.LookingAt].gamedata);
							adjustMoveList();
		...
		// after clicking a game tab the console is not pointed to main but when it is
		// we dont change any other info like LookingAt just the console so we need to
		// have a way of telling when chat is going to main like when you type somethi
	}
	...
	public void makerightclickmainhappen(MouseEvent e) {
		...
		item1.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				setMainConsoleDoc();
				sharedVariables.pointedToMain[gameData.BoardIndex] = true; // when you type our response text. what you
																			// type will go to main. LookingAt stays on
																			// game only console text is effected by
																			// sharedVariables.pointedToMain[gameData.BoardIndex]
		...
		for (int b = 1; b < sharedVariables.maxConsoleTabs; b++) {
			...
			items[a].addActionListener(new ActionListener() {
				public void actionPerformed(ActionEvent e) {
					...
					setMainConsoleDoc();
					sharedVariables.pointedToMain[gameData.BoardIndex] = true; // when you type our response text. what
																				// you type will go to main. LookingAt
																				// stays on game only console text is
																				// effected by
																				// sharedVariables.pointedToMain[gameData.BoardIndex]
	...
	public void makerightclickhappen(MouseEvent e, final int n) {
		...
		saveitem.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				try {
					...
					if (myfile.equals(""))
						return;
					logPgn(sharedVariables.mygame[gameData.LookingAt].iccResultString,
							sharedVariables.mygame[gameData.LookingAt].iccResult, myfile);
		...
		/*
		 * JMenuItem item3 = new JMenuItem("cancel"); menu2.add(item3);
		 */
	...
	void logPgn(String iccresult, String iccresultstring, String fileName) {
		try {
			...
			game += "[Event \"ICC " + wildString + sharedVariables.mygame[gameData.LookingAt].time + " "
					+ sharedVariables.mygame[gameData.LookingAt].inc + isRated + "\"]\r\n";
			...
			public void run() {
				try {
					...
					sharedVariables.gametable[aaa]
							.scrollRectToVisible(sharedVariables.gametable[aaa].getCellRect(row, column, true));
	...
	void dispatchCommand(String myurl) {
		...
		String mycommand = "";
		mycommand = myurl; // .substring(1, myurl.length()-1);// need to figure out why this is -2 not -1,
							// maybe i include the end space which adds a charaacter here when i cut it
		mycommand = mycommand + "\n";
		...
		prefixHandler.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				try {
					...
					// sharedVariables.looking[consoleNumber] + " " + e.getActionCommand());
...
// each listener does any work when a tab is clicked in makehappen() method
		...
		// telnet writes to documents and another board can be looking at this boards
		// game and console and just needs its document which is now stored on the array
		...
		gameconsoles[gameData.BoardIndex].addMouseListener(new MouseListener() {
			public void mouseClicked(MouseEvent e) {
				...
				if (!editor.isEditable()) {
					...
					if (pos >= 0) { // get the element at the pos
...
//String underline="false";
						...
						String myurl = "";
						if (at.containsAttributes(attrs)) {// underline = "true";
															// at has atributes
		...
		Input.addKeyListener(new KeyListener() {
			...
			public void keyPressed(KeyEvent e) {
				...
				if (e.getModifiersEx() == 512
						|| (sharedVariables.operatingSystem.equals("mac") && e.getModifiersEx() == 128)) // alt
				{
				...
				/*
				 * if( aa == 71 && e.getModifiersEx() == 128) // ctrl + g { String myurl
				 * =Input.getText(); Input.setText("");
				 * 
				...
				if (e.getModifiersEx() == 128 && aa == 76)// control 0
				{
					...
					/*
					 * JFrame framer; if(sharedVariables.consoleDebug == true) framer = new
					 * JFrame("" + 1); else framer = new JFrame("" + 0); framer.setSize(100,100);
					...
					if (aa == 79) // 'o' ctrl + 0 -- debug info
					{
						...
						for (index = 0; index < sharedVariables.openBoardCount; index++) {
							try {
								...
								mess += " Exception finding tab were pointing to on index " + index + " ";
							} // end catch
				...
				if (aa == 10) {
					...
					try {
						...
						myoutput output;
						if (sharedVariables.mygame[sharedVariables.gamelooking[gameData.BoardIndex]].myGameNumber != -1
								&& sharedVariables.myServer.equals("ICC"))// primary doesnt work on fics when playing so
																			// for now we dont send it there MA 5-31-10
						{
							output = new myoutput();
							output.data = primary;
							...
							/*
							 * if(sharedVariables.myServer.equals("ICC") && sharedVariables.myname.length()
							 * > 0) { if(sharedVariables.pointedToMain[gameData.LookingAt] == false)
							 * output.data="`g" + gameData.LookingAt + "`" + primary; else output.data="`c0"
							 * + "`" + primary; }
							 */
						...
						if (consoleSubframes[a].isVisible()) {
							if (sharedVariables.looking[a] == i) {
								try {
									...
									/*
									 * if(false == sharedVariables.ConsoleScrollPane[a].getVerticalScrollBar().
									 * getValueIsAdjusting()) {
									 * 
									 * // lets check if the scroll bar is raised up passeed a bottom window that
									 * we'll auto max it if its in
									 * if(sharedVariables.ConsoleScrollPane[a].getVerticalScrollBar().getValue() +
									 * 500 >
									 * sharedVariables.ConsoleScrollPane[a].getVerticalScrollBar().getMaximum())//
									 * scrolled more than 10 lines up dont auto scroll {
									 * 
									 * sharedVariables.ConsoleScrollPane[a].getVerticalScrollBar().setValue(
									 * sharedVariables.ConsoleScrollPane[a].getVerticalScrollBar().getMaximum());//
									 * scrolled mor
									 * 
									 * }// end if }// end if
			...
			void doToolBarCommand(int n) {
				...
				commander.dispatchCommand(n, sharedVariables.gamelooking[gameData.BoardIndex], true, sharedVariables,
						queue);
...
//sharedVariables.ConsoleScrollPane[consoleNumber].getVerticalScrollBar().addMouseWheelListener(new MouseWheelListener()
				...
				if (e.getScrollType() == MouseWheelEvent.WHEEL_UNIT_SCROLL) {
					...
					if (notches < 0) {
						jScrollPane1.getVerticalScrollBar().setValue(
								jScrollPane1.getVerticalScrollBar().getValue() - (e.getScrollAmount() * mult));
						scrollnow = 0;
						...
						int myvalue = 100 + d;
						if (jScrollPane1.getVerticalScrollBar().getValue() + myvalue > jScrollPane1
...

						{
							...
							jScrollPane1.getVerticalScrollBar()
									.setValue(jScrollPane1.getVerticalScrollBar().getMaximum());
					 ...
					 * jScrollPane1.getVerticalScrollBar().getBlockIncrement(1) + " pixels" +
					 * newline;
					 */
					...
					if (notches < 0) {
						jScrollPane1.getVerticalScrollBar()
								.setValue(jScrollPane1.getVerticalScrollBar().getValue() - block);
		...
		jScrollPane1.getVerticalScrollBar().addAdjustmentListener(new AdjustmentListener() {
			public void adjustmentValueChanged(AdjustmentEvent e) {
				...
				// below code tells if they click the up line button and i've over riden its
				// value in blockInc and when adjustment =block inc i dont scroll
				int z = jScrollPane1.getVerticalScrollBar().getValue()
						- jScrollPane1.getVerticalScrollBar().getMaximum();

				int dd = gameconsoles[gameData.BoardIndex].getScrollableBlockIncrement(
						gameconsoles[gameData.BoardIndex].getVisibleRect(), SwingConstants.VERTICAL, -1);
				z = z + dd;
				z *= -1;
				...
				if (scrollnow == 1 && false == jScrollPane1.getVerticalScrollBar().getValueIsAdjusting()) {
					...
					int d = gameconsoles[gameData.BoardIndex].getScrollableBlockIncrement(
							gameconsoles[gameData.BoardIndex].getVisibleRect(), SwingConstants.VERTICAL, -1);
...
// this boards game console always specified by the  array index BoardIndex
	...
	void writeTypedText(String mes) {
		...
		try {
			...
			if (sharedVariables.pointedToMain[gameData.BoardIndex] == false)
				...
				doc = sharedVariables.mydocs[mainConsoleIndex]; // LookingAt always stays on game even if they click
																// main tab so we use this variable , pointedatmain to
																// see if we really want main document

			// doc.insertString(doc.getLength(), mes, null);

			/*
			 * if(sharedVariables.pointedToMain[gameData.BoardIndex] == false) { for(int
			 * a=0; a<sharedVariables.maxGameTabs && a < sharedVariables.openBoardCount;
			 * a++) if(sharedVariables.gamelooking[gameData.BoardIndex]==sharedVariables.
			 * gamelooking[a])
			 */
			if (sharedVariables.pointedToMain[gameData.LookingAt] == false) {
				...
				int maxLinks = 75;
				myDocWriter.processLink(doc, mes, sharedVariables.typedColor, gameData.LookingAt, maxLinks,
						GAME_CONSOLES, attrs, null);
				...
				// writeToConsole(doc, 0); // we need to move this function for general use out
				// of chessbot 4 and out of this file to one place for general use
				// for(int a=0; a<sharedVariables.maxGameTabs && a <
				// sharedVariables.openBoardCount; a++)
				/*
				...
				int maxLinks = 75;
				myDocWriter.processLink(doc, mes, sharedVariables.typedColor, /* gameData.LookingAt */ mainConsoleIndex,
	...
	void setHorizontalLayout() {
		...
		// GroupLayout.DEFAULT_SIZE, 450, Short.MAX_VALUE);// mike commented out
	...
	void setupLargeMenu(final String handle) {
		...
		if (sharedVariables.rightClickMenu.size() > 0) {
			...
			for (int m = 0; m < sharedVariables.rightClickMenu.size() - 1; m++) // size()-1 caue last item is just used
																				// in console now the Tell name
			{
				...
				items[m].addActionListener(new ActionListener() {
					public void actionPerformed(ActionEvent e) {
						...
						if (sharedVariables.rightClickMenu.get(mfinal).equals("Hyperlink")) {
							...
							tellMasterDialog frame = new tellMasterDialog(tripper, false, sharedVariables, theName,
									soundof, channelof);
		...
		/*
		 * JMenuItem item = new JMenuItem("Finger"); item.addActionListener(new
		 * ActionListener() { public void actionPerformed(ActionEvent e) { String name =
		 * gameconsoles[gameData.BoardIndex].getSelectedText(); doCommand("finger " +
		 * name + "\n"); } }); menu3.add(item);
		 * 
		 * 
		 * 
		 * JMenuItem item2 = new JMenuItem("Vars"); item2.addActionListener(new
		 * ActionListener() { public void actionPerformed(ActionEvent e) { String name =
		 * gameconsoles[gameData.BoardIndex].getSelectedText(); doCommand("Vars " + name
		 * + "\n"); } }); menu3.add(item2); JMenuItem item3 = new JMenuItem("Ping");
		 * item3.addActionListener(new ActionListener() { public void
		 * actionPerformed(ActionEvent e) { String name =
		 * gameconsoles[gameData.BoardIndex].getSelectedText(); doCommand("ping " + name
		 * + "\n"); } }); menu3.add(item3); add(menu3);
		 * 
		 * menu3.addSeparator();
		 * 
		 * JMenuItem item4 = new JMenuItem("Match"); item4.addActionListener(new
		 * ActionListener() { public void actionPerformed(ActionEvent e) { String name =
		 * gameconsoles[gameData.BoardIndex].getSelectedText(); doCommand("match " +
		 * name + "\n"); } }); menu3.add(item4);
		 * 
		 * 
		 * 
		 * JMenuItem item5 = new JMenuItem("Assess"); item5.addActionListener(new
		 * ActionListener() { public void actionPerformed(ActionEvent e) { String name =
		 * gameconsoles[gameData.BoardIndex].getSelectedText(); doCommand("assess " +
		 * name + "\n"); } }); menu3.add(item5);
		 * 
		 * JMenuItem item6 = new JMenuItem("Pstat"); item6.addActionListener(new
		 * ActionListener() { public void actionPerformed(ActionEvent e) { String name =
		 * gameconsoles[gameData.BoardIndex].getSelectedText(); doCommand("pstat " +
		 * name + "\n"); } }); menu3.add(item6);
		 * 
		 * 
		 * menu3.addSeparator();
		 * 
		 * 
		 * JMenuItem item7 = new JMenuItem("Observe"); item7.addActionListener(new
		 * ActionListener() { public void actionPerformed(ActionEvent e) { String name =
		 * gameconsoles[gameData.BoardIndex].getSelectedText(); doCommand("observe " +
		 * name + "\n"); } }); menu3.add(item7);
		 * 
		 * 
		 * 
		 * JMenuItem item8 = new JMenuItem("Follow"); item8.addActionListener(new
		 * ActionListener() { public void actionPerformed(ActionEvent e) { String name =
		 * gameconsoles[gameData.BoardIndex].getSelectedText(); doCommand("follow " +
		 * name + "\n"); } }); menu3.add(item8); menu3.addSeparator();
		 * 
		 * JMenuItem item9 = new JMenuItem("History"); item9.addActionListener(new
		 * ActionListener() { public void actionPerformed(ActionEvent e) { String name =
		 * gameconsoles[gameData.BoardIndex].getSelectedText(); doCommand("history " +
		 * name + "\n"); } }); menu3.add(item9);
		 * 
		 * 
		 * 
		 * JMenuItem item10 = new JMenuItem("Liblist"); item10.addActionListener(new
		 * ActionListener() { public void actionPerformed(ActionEvent e) { String name =
		 * gameconsoles[gameData.BoardIndex].getSelectedText(); doCommand("liblist " +
		 * name + "\n"); } }); menu3.add(item10);
		 * 
		 * 
		 * 
		 * JMenuItem item11 = new JMenuItem("Stored"); item11.addActionListener(new
		 * ActionListener() { public void actionPerformed(ActionEvent e) { String name =
		 * gameconsoles[gameData.BoardIndex].getSelectedText(); doCommand("stored " +
		 * name + "\n"); } }); menu3.add(item11);
		 * 
		 */
	 ...
	 * 
	 * //Process p = Runtime.getRuntime().exec(cmdLine); Runtime rt =
	 * Runtime.getRuntime(); if (os.indexOf( "win" ) >= 0) { String[] cmd = new
	 * String[4]; cmd[0] = "cmd.exe"; cmd[1] = "/C"; cmd[2] = "start"; cmd[3] =
	 * myurl;
	 * 
	 * rt.exec(cmd); } else if (os.indexOf( "mac" ) >= 0) {
	 * 
	 * Runtime runtime = Runtime.getRuntime(); if(myurl.startsWith("www."))
	 * myurl="http://" + myurl; String[] args = { "osascript", "-e",
	 * "open location \"" + myurl + "\"" }; try { Process process =
	 * runtime.exec(args); } catch (IOException e) { // do what you want with this
	 ...
	 * 
	 * 
	 * } else { //prioritized 'guess' of users' preference String[] browsers =
	 * {"epiphany", "firefox", "mozilla", "konqueror",
	 * "netscape","opera","links","lynx"};
	 * 
	 * StringBuffer cmd = new StringBuffer(); for (int i=0; i<browsers.length; i++)
	 ...
	 * 
	 * 
	 * }// end else }// end try catch(Exception e) {}
	 * 
	 * 
	 * }
	 */
