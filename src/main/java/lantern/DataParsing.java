...
public class DataParsing {
	...
	void getData(String data) {
		...
		while (temp_top < data.length()) {
			...
			if (dataTop > 0 && icc_data[dataTop] == ')' && icc_data[dataTop - 1] == '\031') {
				...
				if (icc_data[0] == 'a' && icc_data[1] == 'i' && icc_data[2] == 'c' && icc_data[3] == 's'
						&& icc_data[4] == '%' && icc_data[5] == ' ') {
					processLine("fics%");
					reset();

				}
			} else if (dataTop == 5) {
				if (icc_data[0] == 'f' && icc_data[1] == 'i' && icc_data[2] == 'c' && icc_data[3] == 's'
						&& icc_data[4] == '%' && icc_data[5] == ' ') {
					processLine("fics%");
					reset();
	...
	void ficsProcessLine(String data) {
		...
		if (data.startsWith("E1 The feature you're attempting to use requires Full Membership.")) {
			...
			data = "Woops. That's a member's command.  Guests can type in console 'seek' for a quick game, tell 1 my question, and if played, type 'examine -1' to make your last game appear on board to be examined.";
		} else if ((ficsType == NO_TYPE || ficsType == UNKNOWN_TYPE) && data.contains("chessclub.com")
				&& !mySettings.fics) {
			data = "***";
		}
		...
		} else if (ficsType == HISTORY_TYPE && !data.trim().startsWith("History")
				&& !data.trim().startsWith("Opponent")) {
			addHistoryItem(data);
		...
		/*
		 * if((ficsType == HISTORY_LIST) && lineCount == 1) { lastGameListName =
		 * getGameListName();
		 * 
		 * if(lastGameListName.equals(mySettings.whoAmI)) { String search = data;;
		 * 
		 * 
		 * } refreshGameListWithData(data, "none", true);
		 * if(((MainActivity)MainActivity.getAppActivity()).getTabHost().getCurrentTab()
		 * == MainActivity.BOARD_TAB) { if(lastGameListName.equals(mySettings.whoAmI)) {
		 ...
		 * if(((MainActivity)MainActivity.getAppActivity()).getTabHost().getCurrentTab()
		 * == MainActivity.BOARD_TAB) { if(lastGameListName.equals(mySettings.whoAmI)) {
		 * skipShowingGameList = true; refreshGameListWithData(data, "none", false); }
		 * 
		 * }
		 * 
		 * }
		 ...
		 * if(((MainActivity)MainActivity.getAppActivity()).getTabHost().getCurrentTab()
		 * == MainActivity.BOARD_TAB) { String search = data; ;
		 * 
		...
		/*
		 * if(ficsType == SAY_TYPE) { if(lineCount == 1 && spaceSeperatedLine.size() >
		 * 0) { updateTeller(); } if((data.startsWith("(told ") ) && lineCount > 1) {
		 * ficsChatTell2 = ficsChatTell2 + data.trim(); } else { ficsChatTell =
		 * ficsChatTell + data.trim(); } }
		 */

		/*
		 * if(ficsType == SEEK_ADD || ficsType == SEEK_REMOVE || ficsType ==
		 * SEEK_REMOVE_ALL) { [self processSeekData: ficsType : data]; [self
		 * resetParsing]; return; }
		 * 
		 * 
		 */
		...
		if (ficsType == CHANNEL_TELL || ficsType == NOTIFY_TYPE || ficsType == PERSONAL_TELL || ficsType == SHOUT_TELL
				|| ficsType == KIB_TYPE || ficsType == SAY_TYPE /* || ficsType == CSHOUT_TYPE */) {

			if (ficsType == PERSONAL_TELL && lineCount == 1) {
				setLastTeller();
			}

			if ((data.startsWith("(told ") || data.startsWith("(shouted to ") || data.startsWith("(c-shouted to ")
					|| data.startsWith("(kibitzed to") || data.startsWith("(whispered to")) && lineCount > 1) {
		...
		// if(ficsType == UNKNOWN_TYPE || ((ficsType == HISTORY_LIST || ficsType ==
		// JOURNAL_LIST) && !skipShowingGameList))
		if (ficsType != CHANNEL_TELL && ficsType != NOTIFY_TYPE && ficsType != PERSONAL_TELL && ficsType != SHOUT_TELL
				&& ficsType != KIB_TYPE && ficsType != SAY_TYPE && ficsType != HISTORY_TYPE
				&& ficsType != JOURNAL_TYPE) {
			if (data.length() == 1) {
		 ...
		 * if(mySettings.guest) { mySettings.chatLog.
		 * addChat("Just type seek in the console and wait to be matched or go to Board tab's Option menu for the Seek a Game form."
		 * ,"bott_tell"); if(data.startsWith("crazyhouse set.")) { mySettings.chatLog.
		 * addChat("Guests can create a free FICS account if they want at https://www.freechess.org/Register/index.html"
		 * ,"bott_tell"); } else { mySettings.chatLog.
		 * addChat("type tell 1 user's question - Contact ICC Online Help, questions may not be answered immediately."
		 * ,"bott_tell"); }
		 * 
		 * } else { mySettings.chatLog.
		 * addChat("To start playing go to Board tab's Option menu for the Seek a Game form."
		 * ,"bott_tell"); if(data.startsWith("bell set to 0.")) { mySettings.chatLog.
		 * addChat("Members can purchase time in the chessclub's official 'Chess at ICC' app in App Store as an In-App purchase and use the time in this App or on any device."
		 * ,"bott_tell"); } }
		 * 
		 * }
		 */
	...
	void writeOutToShouts(String thetell) {
		...
		processLink2(doc, thetell, sharedVariables.shoutcolor, sharedVariables.shoutRouter.shoutsConsole, maxLinks,
				SUBFRAME_CONSOLES, attrs, cindex, myStyles);
		if (sharedVariables.shoutRouter.shoutsConsole > 0 && sharedVariables.shoutsAlso == true) {
			doc = sharedVariables.mydocs[0];
			processLink2(doc, thetell, sharedVariables.shoutcolor, 0, maxLinks, SUBFRAME_CONSOLES, attrs, cindex,
					myStyles);
		}
	}
	...
	void writeOutToChannel(String theTell, int channelNumber) {
		...
		if ((goTab == false || (sharedVariables.mainAlso[channelNumber] == true))
				&& sharedVariables.qtellController[0][channelNumber] != 2) {
					...
					for (int znumber = 0; znumber < 400; znumber++) {
						...
						if (sharedVariables.console[ztab][znumber] == 1 || ztab == 0) {
							...
							if (sharedVariables.tabStuff[ztab].ForColor == null)
								processLink(doc, theNotifyTell, sharedVariables.ForColor, ztab, maxLinks, subframe_type,
										attrs, null);
							else
								processLink(doc, theNotifyTell, sharedVariables.tabStuff[ztab].ForColor, ztab, maxLinks,
										subframe_type, attrs, null);
							break;
						} // end if print
		...
		try {
			...
			for (int z = 0; z < sharedVariables.openBoardCount; z++) {
				if (mainTelnet.myboards[z] != null)
					if (sharedVariables.boardConsoleType != 0)// dont send tell to board if console disabled, would have
																// later caused it to print twice in main
						if (sharedVariables.mygame[z] != null && (sharedVariables.mygame[z].realname1.equals(name)
								|| sharedVariables.mygame[z].realname2.equals(name))) {

							doc = sharedVariables.mygamedocs[z];
							processLink(doc, thetell, sharedVariables.tellcolor, z, maxLinks, GAME_CONSOLES, attrs,
									myStyles);
						}
			}
...
//sendMessage("Away " + myaway + "\n");// implment fics send
	...
	int getType(String lineData) {
		...
		/*
		 * 
		 * if([spaceSeperatedLine count] > 2) { NSString *tempo = [spaceSeperatedLine
		 * objectAtIndex:0]; NSString *tempo2 = [spaceSeperatedLine objectAtIndex:1];
	...
	boolean isHashKeyTell(String line, ArrayList<String> spaceSeperatedLine) {
		/*
		 * static boolean console = true; static int number = -1; static int userHashKey
		 * = -1;
		 */
	...
	void writeOutToMain(String ficsChatTell) {
		try {
			...
			if (sharedVariables.tabStuff[consoleNumber].ForColor == null)
				processLink2(doc, ficsChatTell, mySettings.ForColor, consoleNumber, maxLinks, SUBFRAME_CONSOLES, attrs,
						cindex2, null);
			else
				processLink2(doc, ficsChatTell, sharedVariables.tabStuff[consoleNumber].ForColor, consoleNumber,
						maxLinks, SUBFRAME_CONSOLES, attrs, cindex2, null);
		} catch (Exception e) {
		}
	}
	...
	void addHistoryItem(String data) {
		...
		try {
			if (line.size() == 18) {
				 ...
				 * String whiteinc, String rated, String ratedType, String wild, String eco,
				 * String status, String color, String mode, tableClass myTable)
				 */
	...
	void addJournalItem(String data) {
		...
		try {
			/*
			 * this one has a lenght of 12 %20: johnas 1573 Liszt 1727 [ br 5 2] E60 NM
			 * 1/2-1/2
			 * 
			 * this one has a lenght of 13 %21: *KeresPaulGM( 1203 *WinterWillia 1203 [ uu 0
			 * 0] B29 --- *
			 */
			if (line.size() == 13 || line.size() == 12) {
				...
				/*
				 * void addSearchLiblistRow(String index, String whiteName, String blackName,
				 * String whiteRating, String blackRating, String date, String time, String
				 * whitetime, String whiteinc, String rated, String ratedType, String wild,
				 * String eco, String status, String color, String mode, String libnote,
				 * tableClass myTable)
				 */
				// 29: + 1753 B 1713 ZwazO [ sr 15 0] A25 Mat Wed Apr 5, 00:50 EDT 2023
				String index = line.get(0).substring(0, line.get(0).length() - 1);
				...
				myItem.addSearchLiblistRow(index, whiteName, blackName, whiteRating, blackRating, date, timeControl,
						wTime, wInc, rated, ratingType, line.get(11), end, result, line.get(9), eco, "",
						mainTelnet.gameList);
	...
	void sendOutChat() {
		/*
		 * if(ficsType == SAY_TYPE) { mySettings.gameChatLog.addChat(ficsChatTell,
		 * "tell"); if(!ficsChatTell2.equals("")) {
		 * mySettings.gameChatLog.addChat(ficsChatTell2, "server_text"); }
		 * if(mySettings.saysInMain) { mySettings.chatLog.addChat(ficsChatTell,"tell");
		 * if(!ficsChatTell2.equals("")) { mySettings.chatLog.addChat(ficsChatTell2,
		 * "server_text"); } consoleManager.updateChat(); }
		 * 
		 * gameConsoleManager.updateChat(); if(mySettings.otherSounds) {
		 ...
		 */try {
			...
			/*
			 * else if(ficsType == CSHOUT_TYPE) { mySettings.chatLog.addChat(ficsChatTell,
			 * "s-shout"); if(!ficsChatTell2.equals("")) {
			 * mySettings.chatLog.addChat(ficsChatTell2, "server_text"); }
			 * consoleManager.updateChat(); } else if(ficsType == NOTIFY_TYPE) {
			 * mySettings.chatLog.addChat(ficsChatTell, "notify");
			 * mySettings.gameChatLog.addChat(ficsChatTell, "notify");
			 * if(!ficsChatTell2.equals("")) { mySettings.chatLog.addChat(ficsChatTell2,
			 * "server_text"); mySettings.gameChatLog.addChat(ficsChatTell2, "notify"); }
			 * consoleManager.updateChat(); gameConsoleManager.updateChat(); }
			 */
		} catch (Exception dui) {
	...
	void seperateLine(String data, ArrayList<String> spaceSeperatedLine1) {
		...
		// [@"abc xyz http://www.abc.com aaa bbb ccc" substringWithRange:NSMakeRange(8,
		// 18)]
	...
	void checkIfLoggedIn() {
		...
		if (spaceSeperatedLine.size() > 4) {
			...
			if (line2.equals("Starting") && line3.equals("FICS") && line4.equals("session") && line5.startsWith("as")) {
				if (line6.equals("")) {
	...
	void sendConnectUserMessages() {
		writeOutToMain(
				"To resize the board and console click in bottom right corner and drag were it shows 2 lines \"//\" (by bottom right corner of typing input field)\n");
		if (mySettings.guest) {
	...
	void processChannels(String channels) {
		...
		if (mySettings.hasSettings == false && !setChannelTabs) {
			// mainTelnet.writeToConsole("Welcome to Pearl");
			// //sharedVariables.newUserMessage);
			setUpNewUserTabs();
			setChannelTabs = true;
		}
	...
	public class Datagram1 {
		...
		Datagram1(String s) {
			try {
				 ...
				 * "trying to parse 2, p.length =" + p.length() + " p is: " + p + "\n", null);
				 * 
				 * 
	...
	boolean processGameLine(String newdata) {
				...
				if (gameExists) {
					if (backedUp) {
						...
						if (checkIfIllegalPlayedMoveWasMade(styleLine)) {
							...
							if (getMyRelation(newdata) == 2
									&& checkGameStatus(styleLine.getGameNumber()) == mySettings.STATE_OBSERVING
									&& !processingLater) {
		...
		/*
		 * 
		 * if(isPrimaryMessage(newdata)) { return true; }
		 */
		...
		/*
		 * 
		 * if(newdata.startsWith("<b1>")) { // <b1> game 818 white [] black [PP]
		 * ArrayList<String> spaceArray = new ArrayList<>(); seperateLine(newdata,
		 * spaceArray); String number = "9999999"; if(spaceArray.size() > 6) { number =
		 * spaceArray.get(2); } GameState openGame = getAnOpenGameState(false, number,
		 * mySettings); if(openGame != null && openGame.gameNumber.equals(number)) {
		 ...
		 * if(newdata.startsWith("<12>"))
		 * 
		 * { String number = getGameNumber(newdata); GameState openGame =
		 * getAnOpenGameState(false, number, mySettings); if (openGame == null ||
		 * !openGame.gameNumber.equals(number) || openGame.relationToGame.equals("-3"))
		 * { GAME_TYPE = GAME_START; } if (GAME_TYPE != NO_TYPE) { GameState tempo =
		 * processGameType(GAME_TYPE, number); if (GAME_TYPE == GAME_START && tempo !=
		 * null) { openGame = tempo; openGame.gameNumber = number; startClock = true; }
		 * }
		 * 
		 * 
		 * openGame.setStyle12Board(newdata, checkIfPlaying(openGame));
		 * openGame.runningClockUpdate(openGame.sideToMove, getMilliSeconds(openGame),
		 * "1");
		 * 
		 * if(GAME_TYPE == GAME_START) { for(int aa = 0; aa < 64; aa++) {
		 * if(openGame.flipType.equals("1")) { openGame.initialScrollBoard[aa] =
		 * openGame.board[63 - aa]; } else { openGame.initialScrollBoard[aa] =
		 * openGame.board[aa]; }
		 * 
		 * } if(openGame.relationToGame.equals("1") ||
		 * openGame.relationToGame.equals("-1")) { if(mySettings.gameSounds) {
		 * MainActivity.playSound("gamestart"); } dismissSeekGraph(); }
		 * if(!lastGameStartString.equals("")) {
		 * setGameStartParamsAsNeeded(lastGameStartString); lastGameStartString = ""; }
		 * else if(!mySettings.fics) { setICCGameStartParamsAsNeeded(openGame); }
		 * 
		 * //remove seek graph if(openGame.relationToGame.equals("-1") ||
		 * openGame.relationToGame.equals("1")) { MainActivity.mySquares.mygame =
		 * openGame; mygame = openGame; MainActivity main =
		 * (MainActivity)MainActivity.getAppActivity(); int tab =
		 * main.getTabHost().getCurrentTab(); if(tab == MainActivity.BOARD_TAB) {
		 * ICSBoard board = (ICSBoard)
		 * main.getLocalActivityManager().getCurrentActivity();
		 * setStockfishButtonVisiblity(board, false); } }
		 * 
		 * 
		 * 
		 * 
		 * } else { GAME_TYPE = SEND_MOVE; processGameType(SEND_MOVE, number); }
		 * 
		 * if(openGame.relationToGame.equals("1") && !openGame.premoveMade.equals("")) {
		 * OutputDataClass data = new OutputDataClass(); data.sendData =
		 * openGame.premoveMade; openGame.premoveMade = ""; openGame.premoveFrom = -1;
		 * openGame.premoveTo = -1; sendQueueConsole.add(data); }
		 * 
		 * if(startClock && mygame == openGame && !mygame.relationToGame.equals("2") &&
		 * !mygame.relationToGame.equals("-3")) { mygame.clockRunning = "1";
		 * openGame.runningClockUpdate(openGame.sideToMove, getMilliSeconds(openGame),
		 * "1"); MainActivity.mySquares.startStopClock("1"); }
		 * 
		 * new Handler(Looper.getMainLooper()).post(new Runnable() {
		 * 
		 * @Override public void run() { try { MainActivity.mySquares.invalidate(); }
		 * catch (Exception dui) { //Log.d("TAG",
		 * "setStockfishButtonVisiblity: mike exceptoin on visiblity " +
		 * dui.getMessage()); } } });
		 * 
		 * }
		 * 
		...
		try {
			...
			try {
...

				{
					...
					if (!theTell2.equals("")) {
						processLink2(doc, theTell + "\n" + theTell2 + "\n", sharedVariables.tellcolor, direction,
								maxLinks, SUBFRAME_CONSOLES, attrs, cindex2, null);
					} else {
						processLink2(doc, theTell + "\n", sharedVariables.tellcolor, direction, maxLinks,
								SUBFRAME_CONSOLES, attrs, cindex2, null);
					}

					try {
						if (sharedVariables.tellsToTab == true && sharedVariables.switchOnTell == true
								&& him == false) {
							FocusOwner whohasit = new FocusOwner(sharedVariables, mainTelnet.consoleSubframes,
									mainTelnet.myboards);
							int xxx = mainTelnet.getCurrentConsole();
							mainTelnet.consoleSubframes[sharedVariables.tellconsole]
									.makeHappen(sharedVariables.tellTab);
				...
				for (int z = 0; z < sharedVariables.openBoardCount && him == false; z++) {
					if (mainTelnet.myboards[z] != null) {
						...
						if (sharedVariables.mygame[z] != null
								&& (sharedVariables.mygame[z].state == sharedVariables.STATE_PLAYING
										|| (sharedVariables.mygame[z].state == sharedVariables.STATE_OVER
												&& (sharedVariables.mygame[z].realname1.toLowerCase()
														.equals(name.toLowerCase())
														|| sharedVariables.mygame[z].realname2.toLowerCase()
																.equals(name.toLowerCase()))))) {
							...
							if (sharedVariables.boardConsoleType != 0) {
								...
								if (!theTell2.equals("")) {
									processLink(doc, theTell + "\n" + theTell2 + "\n", sharedVariables.tellcolor,
											number, maxLinks, consoleType, attrs, myStyles);
								} else {
									processLink(doc, theTell + "\n", sharedVariables.tellcolor, number, maxLinks,
											consoleType, attrs, myStyles);
								}
							}
					...
					if (mainTelnet.myboards[z] != null)
						...
						if (sharedVariables.mygame[z] != null && sharedVariables.mygame[z].myGameNumber == gameNumber) {
							...
							if (sharedVariables.boardConsoleType == 0) {
								...
								if (!theTell2.equals("")) {
									processLink2(doc, theTell + "\n" + theTell2 + "\n", sharedVariables.kibcolor, 0,
											maxLinks, SUBFRAME_CONSOLES, attrs, cindex2, null);
								} else {
									processLink2(doc, theTell + "\n", sharedVariables.kibcolor, 0, maxLinks,
											SUBFRAME_CONSOLES, attrs, cindex2, null);
								}
							} else {

								if (!theTell2.equals("")) {
									processLink(doc, theTell + "\n" + theTell2 + "\n", sharedVariables.kibcolor, number,
											maxLinks, consoleType, attrs, myStyles);
								} else {
									processLink(doc, theTell + "\n", sharedVariables.kibcolor, number, maxLinks,
											consoleType, attrs, myStyles);
								}
							}
	 ...
	 * 
	 * 
	 * if(GAME_TYPE == GAME_START) { GameState openGame = null; if(mygame != null) {
	 * boolean isPlaying = false; if(checkIfPlaying(mygame)) { isPlaying = true; }
	 * if(checkIfObserving()) { mygame.observingGameNumber = number;
	 * 
	 * } openGame = getAnOpenGameState(true , number , MainActivity.mySettings);
	 * 
	 * 
	 * openGame.gameResult = ""; openGame.premoveMade = ""; openGame.premoveFrom =
	 * -1; openGame.premoveTo = -1;
	 * 
	 * try { if(!isPlaying) { mygame = openGame; MainActivity.mySquares.mygame =
	 * openGame;
	 * 
	 * } MainActivity main = (MainActivity) MainActivity.getAppActivity(); int tab =
	 * main.getTabHost().getCurrentTab(); if(tab == MainActivity.BOARD_TAB) {
	 * ICSBoard board = (ICSBoard)
	 * main.getLocalActivityManager().getCurrentActivity();
	 * turnOffStockfishIfOn(board); } new Handler(Looper.getMainLooper()).post(new
	 * Runnable() {
	 * 
	 * @Override public void run() { try { MainActivity.mySquares.invalidate(); }
	 * catch (Exception dui) { //Log.d("TAG",
	 * "setStockfishButtonVisiblity: mike exceptoin on visiblity " +
	 * dui.getMessage()); } } }); } catch(Exception dui) {
	 * 
	 * }
	 * 
	 * 
	 ...
	 * 
	 * GameState openGame = getAnOpenGameState(false , number ,
	 * MainActivity.mySettings); if(openGame != null) { if(mySettings.fics) {
	 ...
	 * 
	 * 
	 * if(GAME_TYPE == GAME_ENDED) { GameState openGame = getAnOpenGameState(false ,
	 * number , MainActivity.mySettings); if(openGame != null) { if(mySettings.fics
	 * && !openGame.relationToGame.equals("0")) { sendToFICS("$iset seekinfo 1"); }
	 * openGame.myGameEnded(); // set result and my relation to game
	 * openGame.gameResult = "*"; openGame.relationToGame = "-3";
	 * if(openGame.observingGameNumber.equals(mygame.observingGameNumber)) {
	 * openGame.observingGameNumber = ""; } //[boardvc.mysquares setBackground];
	 * if(openGame == mygame) { openGame.clockRunning = "0";
	 ...
	 * ICSBoard board = (ICSBoard)
	 * main.getLocalActivityManager().getCurrentActivity();
	 * if(!board.amPlayingAnyGame()) { setStockfishButtonVisiblity(board, true); } }
	 * new Handler(Looper.getMainLooper()).post(new Runnable() {
	 * 
	 * @Override public void run() { try { MainActivity.mySquares.invalidate(); }
	 * catch (Exception dui) { //Log.d("TAG",
	 ...
	 * return null; }
	 * 
	 * void turnOffStockfishIfOn(final ICSBoard board) { new
	 * Handler(Looper.getMainLooper()).post(new Runnable() {
	 * 
	 * @Override public void run() { try { board.setConsoleEngineMode(false); }
	 * catch (Exception dui) { //Log.d("TAG",
	 * "setStockfishButtonVisiblity: mike exceptoin on visiblity " +
	 * dui.getMessage()); } } }); }
	 * 
	 ...
	 * { new Handler(Looper.getMainLooper()).post(new Runnable() {
	 * 
	 * @Override public void run() { try { board.setStockfishButtonVisible(visible);
	 * if(!visible) { board.setConsoleEngineMode(visible); } } catch (Exception dui)
	 * { //Log.d("TAG", "setStockfishButtonVisiblity: mike exceptoin on visiblity "
	 * + dui.getMessage()); } } }); }
	 * 
	 * void playMoveSoundAsNeeded(GameState openGame) {
	 */
	/*
	 * my relation to this game: -3 isolated position, as in "ref 3" or sposition -2
	 * observing examined game 2 the examiner of this game -1 I am playing, it's the
	 * opponent's move 1 I am playing and it's my move 0 observing played game
	 */
	/*
	 * if(mySettings.gameSounds && openGame == MainActivity.mySquares.mygame) {
	 * if(openGame.relationToGame.equals("1") ||
	 * openGame.relationToGame.equals("-1") || openGame.relationToGame.equals("2"))
	 * { MainActivity.playSound("move"); } else if(mySettings.observeGameSounds) {
	 * if(openGame.relationToGame.equals("-2") ||
	 * openGame.relationToGame.equals("0")) { MainActivity.playSound("move"); } } }
	 */
	// }
	...
	/*
	 * 
	 * GameState getAnOpenGameState(boolean new1, String gameNumber, SharedSettings
	 * mySettings) {
	 * 
	 * GameState theGame = null;
	 * 
	 * if(new1) { removeGameThatEnded(); theGame = new GameState();
	 * theGame.mySettings = mySettings; openGames.add( theGame);
	 * 
	 * 
	 * 
	 * } else { try { for(int a = 0; a < openGames.size(); a++) { if
	 * (openGames.get(a).gameNumber.equals(gameNumber)) { theGame =
	 * openGames.get(a); break;
	 * 
	 * } }
	 * 
	 * } catch (Exception exception) { GameState myGame = new GameState();
	 * myGame.mySettings = mySettings; return myGame; } finally { // do something to
	 * keep the program still running properly } } return theGame;
	 * 
	 * }
	 * 
	 * void removeGameThatEnded() { try { GameState theGame = null;
	 * ArrayList<GameState> mygames = openGames; for(int a =0; a < mygames.size();
	 * a++) { GameState theGame2 = mygames.get(a); if
	 * (theGame2.relationToGame.equals("-3")) { mygames.remove(a); return; } } }
	 * catch (Exception dui) { } }
	 * 
	 * boolean checkIfPlaying(GameState mygame) { boolean amPlaying = false;
	 * if(mygame != null) {
	 * 
	 * if(mygame.relationToGame.equals("1") || mygame.relationToGame.equals("-1")) {
	 * if(mygame.clockRunning.equals("1" )) amPlaying = true;
	 * 
	 * } } return amPlaying; }
	 */
	...
	/*
	 * String getMilliSeconds(GameState openGame) { int seconds = 0;
	 * if(openGame.sideToMove.equals("W")) { try { seconds =
	 * Integer.parseInt(openGame.whiteClockSeconds); } catch(Exception dui) {}; }
	 * else { try { seconds = Integer.parseInt(openGame.blackClockSeconds); }
	 * catch(Exception dui) {}; } seconds *=1000; return "" + seconds;
	 * 
	 * }
	 * 
	 */
		...
		if (newdata.startsWith("You are no longer examining game ")) {
			...
			if (spaceArray.size() > 3) {
				...
				if (line1.equals("{Game")) {
					...
					for (int a = 0; a < mySettings.mygame.length; a++) {
						if (mySettings.mygame[a] != null && mySettings.mygame[a].state == mySettings.STATE_PLAYING
								&& number.equals("" + mySettings.mygame[a].myGameNumber)) {
	 ...
	 * 
	 * try { GameStartData data =
	 * (GameStartData)gameStartMap.get(openGame.gameNumber); if(data != null) {
	 * String wElo = data.whiteElo; String bElo = data.blackElo; String wTitle =
	 * data.whiteTitle; String bTitle = data.blackTitle; String rType =
	 * data.ratingType; String r = data.rated; String wild = data.wild;
	 * 
	 * openGame.setGameStartedParmsFics(rType, r, wElo, bElo, wTitle, bTitle, wild);
	 * for(int a = 0; a < data.moves.size(); a++) {
	 * openGame.setLastFromTo(data.moves.get(a)); } } } catch (Exception dui) {
	 * 
	 * }
	 * 
	 * }
	...
	void setGameStartParamsAsNeeded(String data) {
		...
		// -(void) setGameStartedParmsFics: (NSString *) rType : (NSString *) r :
		// (NSString *) wElo : (NSString *) bElo : (NSString *) wTitle : (NSString *)
		// bTitle ;
		if (found) {
			...
			if (found) {
				...
				gamestate openGame = null;
				for (int a = 0; a < mySettings.mygame.length; a++) {
					if (mySettings.mygame[a] != null && mySettings.mygame[a].realname1.equals(tempo1)
							&& mySettings.mygame[a].realname2.equals(tempo2)) {
						openGame = mySettings.mygame[a];
						found = true;
						break;
					}
	...
	boolean isExamineInfo(String data, Channels mySettings) {
		...
		if (data.startsWith("You're at the end of the game.")
				|| data.startsWith("You're at the beginning of the game.")) {
			return true;
		}
		ArrayList<String> spaceArray = new ArrayList<>();
		seperateLine(data, spaceArray);
					...
					for (int a = 0; a < mySettings.mygame.length; a++) {
						if (mySettings.mygame[a] != null) {
							if (mySettings.mygame[a].state == mySettings.STATE_EXAMINING
									&& mySettings.mygame[a].myGameNumber == num) {
								return true;
							}
						}
					}
	...
	void parseForwardBackward(ArrayList<String> spaceArray) {
		/*
		 * 23 DG_BACKWARD Form: (gamenumber backup-count) A player backed up in an
		 * examined game. Also generated by the "revert" command.
		 * 
		 */
	...
	/*
	 * String getGameListName() { String name = ""; if(mySettings.fics) {
	 * if(spaceSeperatedLine.size() > 2) { String tempo = spaceSeperatedLine.get(2);
	 * if (tempo.endsWith(":") && tempo.length() > 2) { return tempo.substring(0,
	 * tempo.length() - 1); } } } else { if(spaceSeperatedLine.size() > 3) { String
	 * tempo = spaceSeperatedLine.get(3); if (tempo.endsWith(":") && tempo.length()
	 * > 2) { return tempo.substring(0, tempo.length() - 1); } } }
	 * 
	 * return name; }
	 * 
	 * String getGameListCommand(String data) { String tempo = "$examine " +
	 * lastGameListName; if(!mySettings.fics) { tempo = "multi examine " +
	 * lastGameListName; } tempo = tempo + " "; // seperateLine:(NSString*) data :
	 * (NSMutableArray*) spaceSeperatedLine1 ArrayList<String> myArray = new
	 * ArrayList<>(); seperateLine(data,myArray); if(myArray.size() > 1) { String
	 * item1 = myArray.get(0); if(item1.endsWith(":") && item1.length() > 1) {
	 * String command = tempo + item1.substring(0, item1.length() -1); return
	 * command; } else { return ""; } }
	 * 
	 * return tempo; }
	 * 
	 * void refreshGameListWithData(final String data, final String command, final
	 * boolean clearData) {
	 * 
	 * new Handler(Looper.getMainLooper()).post(new Runnable() {
	 * 
	 * @Override public void run() { try { if(ICSBoard.gamesAdapter != null) {
	 * if(clearData) { mySettings.currentGameListing.clear();;
	 ...
	 * void updateTeller() {
	 * 
	 * if(spaceSeperatedLine.size() > 0) { String tempo = spaceSeperatedLine.get(0);
	 * if(tempo.contains("(")) { int i = tempo.indexOf("("); if(i > 0) { tempo =
	 * tempo.substring(0, i); } } else if(tempo.contains("[")) { int i =
	 * tempo.indexOf("["); if(i > 0) { tempo = tempo.substring(0, i); } } tempo =
	 * "/tell " + tempo + "! "; mySettings.tellManager.addName(tempo);
	 * 
	 * } MainActivity main = (MainActivity) MainActivity.getAppActivity(); int tab =
	 * main.getTabHost().getCurrentTab(); if(tab == MainActivity.CONSOLE_TAB) {
	 * ICSConsole console = (ICSConsole)
	 * main.getLocalActivityManager().getCurrentActivity();
	 * console.setTellerVisibility(); } }
	 * 
	 * boolean isPrimaryMessage(String data) {
	 * if(data.startsWith("Changing your primary observed game to ")) { return true;
	 * }
	 * 
	 * if(data.startsWith("Your primary game is now ")) { return true; }
	 * if(data.startsWith("Game")) { ArrayList<String> spaceArray = new
	 * ArrayList<>(); seperateLine(data, spaceArray); if(spaceArray.size() > 6) {
	 ...
	 * if(spaceArray.get(6).trim().equals("game.")) return true; } }
	 * 
	 * 
	 * return false; }
	 */
		...
		try {
			...
			if (ficsType == SEEK_ADD) {
				...
				if (sRatingType.equals("standard")) {
					...
					sRatingType = "Unknown";
					// NSLog(@"made rating type unkown for type %@", [self
					// getSeekArgument:[spaceSeperatedLine objectAtIndex:8]] );
				}
				...
				if (mySettings.graphData != null) {
					mySettings.graphData.addSeek(sIndex, sName, sTitles, sRating, sProvisional, sWild, sRatingType,
							sTime, sInc, sRated, sRange, sColor, sFormula, sManual, mainTelnet.notifyList);
				}
	...
	void dismissSeekGraph() {
		/*
		 * MainActivity main = (MainActivity) MainActivity.getAppActivity(); int tab =
		 * main.getTabHost().getCurrentTab(); if(tab == MainActivity.BOARD_TAB) { final
		 * ICSBoard board = (ICSBoard)
		 * main.getLocalActivityManager().getCurrentActivity(); new
		 * Handler(Looper.getMainLooper()).post(new Runnable() {
		 * 
		 * @Override public void run() { try { if(ICSBoard.seekGraphWindow != null) {
		 * board.dismissSeekGraph(); } } catch (Exception dui) { //Log.d("TAG",
		 * "setStockfishButtonVisiblity: mike exceptoin on visiblity " +
		 * dui.getMessage()); } } });
		 * 
		 * }
	...
	boolean illegalMessage(String message, Channels sharedVariables) {
		try {
			for (int a = 0; a < mySettings.mygame.length; a++) {
				if (mySettings.mygame[a] != null) {
					if (mySettings.mygame[a].state == mySettings.STATE_PLAYING
							|| mySettings.mygame[a].state == mySettings.STATE_EXAMINING) {
						newBoardData temp = new newBoardData();
						temp.dg = 77;
	...
	void gameMessage(String message) {
		try {
			...
			if (spaceArray.get(0).startsWith("Creating")) {
				...
				for (int a = 0; a < mySettings.mygame.length; a++) {
					if (mySettings.mygame[a] != null
							&& mySettings.mygame[a].realname1.toLowerCase().equals(spaceArray.get(1).toLowerCase())) {
						temp.arg1 = "" + mySettings.mygame[a].myGameNumber;
						break;
					}
				}
			}
			if (message.startsWith("You're at the end of the game.")
					|| message.startsWith("You're at the beginning of the game.")) {
				for (int a = 0; a < mySettings.mygame.length; a++) {
	...
	void gameStarted(Style12Struct myGameStruct, String wild) {
		...
		temp.arg8 = "" + myGameStruct.getIncrement();
		temp.arg11 = "0";
		temp.arg13 = "";
		temp.arg14 = "";
		temp.arg16 = "";
		...
		// myboards[gamenum].gameStarted(temp.arg1, temp.arg2, temp.arg3, temp.arg4,
		// temp.arg5, //temp.arg6, temp.arg7, temp.arg8, temp.arg11, temp.arg13,
		// temp.arg14, temp.arg16, //temp.arg17, temp.type); // pass game number
	...
	void setGameStartedParmsFics(String myGameNumber, String rType, String r, String wElo, String bElo, String wTitle,
			String bTitle) {
		newBoardData temp = new newBoardData();
		temp.dg = 250;
			...
			for (int a = 0; a < mySettings.mygame.length; a++) {
				if (mySettings.mygame[a] != null && mySettings.mygame[a].myGameNumber == myGameStruct.getGameNumber()) {
					if (mySettings.mygame[a].style12Boards.size() > 0
							&& (mySettings.mygame[a].state == mySettings.STATE_PLAYING
									|| mySettings.mygame[a].state == mySettings.STATE_EXAMINING)) {
						String last = mySettings.mygame[a].style12Boards
								.get(mySettings.mygame[a].style12Boards.size() - 1);
	...
	boolean checkIfGameExists(int gameNumber) {
		/*
		 * int gameNum = -1; try { gameNum = Integer.parseInt(gameNumber); }
		 * catch(Exception dui) { return true; }
		 */
		for (int a = 0; a < mySettings.mygame.length; a++) {
		...
		try {
			...
			if (sharedVariables.channelNamesList.size() < sharedVariables.maxConsoleTabs - 1) {
				...
				for (int li = 0; li < sharedVariables.channelNamesList.size(); li++) {
					sharedVariables.console[li + 1][Integer
							.parseInt(sharedVariables.channelNamesList.get(li).channel)] = 1;
			...
			for (int z = 1; z < sharedVariables.openConsoleCount - 1; z++)
				if (sharedVariables.console[z][221] == 1)
					...
					asetter.createConsoleTabTitle(sharedVariables, z, mainTelnet.consoleSubframes, "");// last argument
																										// tab name
		} catch (Exception dui) {
		}
	...
	void populateEventData() {
		String eventData[] = { "tell puzzlebot getmate", "tell puzzlebot getstudy2", "tell puzzlebot getmate1",

				"tell endgamebot play kpk", "tell endgamebot play kbnk", "tell endgamebot play kbbk",

				"tell endgamebot play kqk", "tell endgamebot play krpkr", "tell endgamebot play krk",

				"tell endgamebot play kqkr", "tell endgamebot play kppkp", "tell endgamebot play kpkp",

				"tell endgamebot play kqpkq", "tell endgamebot play knnkp", "tell endgamebot play kppkpp",
		...
		for (int a = 0; a < eventData.length; a++) {
			if (!addedPuzzleInfo && eventData[a].contains("puzzle")) {
				mainTelnet.eventsList.addToEvents(eventData[a], "" + a, eventData[a], "",
						mySettings.addHashWrapperToLookupUser("$finger puzzlebot\n"));
				addedPuzzleInfo = true;
			} else if (!addedEndgameInfo && eventData[a].contains("endgame")) {
				mainTelnet.eventsList.addToEvents(eventData[a], "" + a, eventData[a], "",
						mySettings.addHashWrapperToLookupUser("$finger endgamebot\n"));
	...
	void startSound() {
		...
		/*
		 * try { FicsSoundPlayer sound = new FicsSoundPlayer(); sound.sharedVariables =
		 * mySettings; Thread soundThread = new Thread(sound); soundThread.start();
		 * 
		 * } catch(Exception gam){}
		 */
	}
