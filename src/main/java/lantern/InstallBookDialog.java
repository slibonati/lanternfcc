			...
			public void mousePressed(MouseEvent e) {
				try {
					...
					if (!file.exists() && !file.isDirectory()) {
						...
						if (installType == mediocreChess5) {
							setPaneText(
									"Mediocre Chess V0.5 succesfully installed.  Go to Options / Other Engines / Analyze With Mediocre Chess v0.5 again to open it.");
						} else if (installType == cuckooChess112) {
							setPaneText(
									"Cuckoo Chess 1.12 succesfully installed.  Go to Options / Analyze With Cuckoo Chess 1.12 again to open it.");
						} else if (installType == stockfish8) {
							setPaneText(stockfish + " succesfully installed.  Go to Options / Analyze With " + stockfish
									+ " again to open it.");
						}

						else {
							setPaneText(
									"Opening book succesfully installed.  Go to Options / Opening Book again to open it.");
						}
					...
					setPaneText(
							"There was an error installing. Does  Lantern have permission to write to this folder?");
					changeButtonsVisiblity();
				}
