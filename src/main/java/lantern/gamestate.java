...
//engineFen+=" w KQkq - 0";
	...
	void readFen2(String fen, int board[]) {
		...
		int a, n;
		if (state != 2)
			return; // we want to use the move list for now. the fen on observing in middle seems to
					// be fen at middle of game
		for (a = 0; a < 64; a++)
			board[a] = 0;
		...
		// now reverse
		/*
		 * int [] board9; board9=new int[64];
		 * 
		 * for(a=0; a<64; a++) board9[a]=board[a];
		 * 
		 * for(a=0; a<8; a++) for(int z=0; z<8; z++) { board[a*8 + z] = board9[a*8 + 7
		 * -z]; }
		 */
	}
	...
	void getSliderBoard(int num, int sliderboard[]) {
		...
		for (int a = 1; a <= num; a++)
			if (wild != 16 || moveListFrom[a - 1] != -1)
				makeslidermove(moveListFrom[a - 1], moveListTo[a - 1], movePromote[a - 1], 1, castleCaptures[a - 1],
						a - 1, sliderboard);
			else {
				if (moveListTo[a] == -1)
...
// prom is any promotion character, i.e 'Q'. i think type that is returnerd is for sound.
	int makemove(int from, int to, char prom, int reload, int castleCapture, String algabraicMove)// h8 = 1 in checkers
	{
		...
		{
			if ((state != 1 || (state == 1 && board[from] != 0)) && (state != 2 || board[from] != 0)) // state
																										// 0=observing
																										// 1=playing
																										// 2=examining
			{
			...
			if (from == blank)
				...
				board[to] = -from;
			type = 1;
			copyBoard();// this should hit at end but for some reason its not getting there with
						// crazyhouse move. need to investigate
		}
		if (reload == 0) {
			moveListTo[movetop] = to;
			moveListFrom[movetop] = from;
	...
	void makeCheckerCapture(int from, int to, int board2[]) {
		...
		if (to % 8 == 0)
			toCol--;
		/*
		 * // works with me white on bottom not black if((fromCol %2 == 0 && iflipped ==
		 * 0) ||(fromCol %2 == 1 && iflipped == 1) ) from--; if((toCol %2 == 0 &&
		 * iflipped == 0) ||(toCol %2 == 1 && iflipped == 1)) to--;
		 */
		if (fromCol % 2 == 0)
			from--;
		if (toCol % 2 == 0)
	...
	void makeslidermove(int from, int to, char prom, int reload, int castleCapture, int movetop, int board[]) {
		...
		if (from == to && from == 0 && wild != 22)
			return; // fics move list could have empty moves since we dont get moves before we start
					// observing.
		...
		if (from >= 0) // not crazyhouse drop
		{
			...
			if ((turn % 2 != check || state != 1 || (state == 1 && board[from] != 0))
					&& (state != 2 || board[from] != 0)) {
				if (wild == 27 && board[to] != 0)
					makeAtomicCaptures(board, from, to);
				board[to] = board[from];
				if (from != to)// so we can send dummy moves like black first
...
// diagonal
	...
	void computeHash() {
		try {
			...
			/*
			 * if(self->rights.wk == 1) key = key ^ hashwk; if(self->rights.wq == 1) key =
			 * key ^ hashwq; if(self->rights.bk == 1) key = key ^ hashbk; if(self->rights.bq
			 * == 1) key = key ^ hashbq;
			 */
	...
	String getStockfishFen() {
		...
		return fen;
		/*
		 * if(enpassantSquare.equals("")) { fen = fen + " - 0"; } else { fen = fen +
		 * " "; fen = fen + enpassantSquare; fen = fen + " 0"; } return fen;
		 */
	}
