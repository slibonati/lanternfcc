...
class gameboardControlsPanel extends JPanel {
	...
	public void paintComponent(Graphics g) {
		...
		try {
			...
			/*
			 * int Width = getWidth(); int Height = getHeight(); if
			 * (sharedVariables.useLightBackground == true)
			 * g.drawImage(graphics.boards[sharedVariables.boardType][graphics.light], 0, 0,
			 * Width, Height, this); else
			 * setBackground(sharedVariables.boardBackgroundColor);
			 */
			...
			if (oldLooking != gameData.LookingAt || oldcountry1 != sharedVariables.mygame[gameData.LookingAt].country1
			...
			if (sharedVariables.mygame[gameData.LookingAt].iflipped == 1) {
				...
				topNameDisplay.setText(sharedVariables.mygame[gameData.LookingAt].name1
						+ sharedVariables.mygame[gameData.LookingAt].country1 + whiteCount);
				botNameDisplay.setText(sharedVariables.mygame[gameData.LookingAt].name2
						+ sharedVariables.mygame[gameData.LookingAt].country2 + blackCount);
			...
			if (sharedVariables.showMaterialCount == true
				...
				if (sharedVariables.mygame[gameData.LookingAt].wild == 0
					...
					listing += " " + sharedVariables.mygame[gameData.LookingAt].whiteMaterialCount + " - "
							+ sharedVariables.mygame[gameData.LookingAt].blackMaterialCount;
			gameListingDisplay.setText(listing);
			setLastMove();

			if (sharedVariables.mygame[gameData.LookingAt].state == sharedVariables.STATE_PLAYING) {// make draw abort
																									// resign buttons
																									// visible
	...
	gameboardControlsPanel(Gameboard myboard1, gamestuff gameData1, Channels sharedVariables1,
			...
			public void mousePressed(MouseEvent e) {
				...
				if (sharedVariables.mygame[gameData.LookingAt].state == sharedVariables.STATE_EXAMINING) {
					...
					if (loc < max) {
						...
						/*
						 * if(loc == max) { int [] slidingBoard = new int[64];
						 * sharedVariables.mygame[gameData.BoardIndex].getSliderBoard(sharedVariables.
						 * moveSliders[gameData.BoardIndex].getValue(), slidingBoard); }
						 */
		...
		/*
		 * buttonPanel.add(backwardEnd); buttonPanel.add(backward);
		 * buttonPanel.add(forward); buttonPanel.add(forwardEnd);
		 */
		...
		if (sharedVariables.moveSliders[gameData.BoardIndex] == null) {
			...
			sharedVariables.moveSliders[gameData.BoardIndex].setPreferredSize(new Dimension(25, 75)); // was 75 25 for
																										// horizontal
																										// oritntation
			...
			/*
			 * sharedVariables.moveSliders[gameData.BoardIndex].addMouseListener (new
			 * MouseAdapter() { public void mousePressed(MouseEvent e) {} public void
			 * mouseReleased(MouseEvent e) { myboard.mypanel.repaint(); adjustMoveList(); }
			 * public void mouseEntered (MouseEvent me) {} public void mouseExited
			 * (MouseEvent me) {} public void mouseClicked (MouseEvent me) {} });
			 */
		...
		if (sharedVariables.mygametable[gameData.BoardIndex] == null) {
			...
			sharedVariables.mygametable[gameData.BoardIndex]
					.createMoveListColumns(sharedVariables.mygame[gameData.BoardIndex].wild);
			sharedVariables.gametable[gameData.BoardIndex] = new JTable(
	...
	private void resizeColumns() {
		// Use TableColumnModel.getTotalColumnWidth() if your table is included in a
		// JScrollPane
	...
	void makeAndreysLayout() {
		...
		/*
		 * int myOverallWidth = getControlLength(); //width of this panel int
		 * myOverallHeight = getControlHeight(); setAndreyFontSize(n); pass in int n
		 * with font size ( effects game board font now not clock
		 * 
		 * if(myOverallWidth < 250) // default minimum width is 235
		 * setAndreyFontSize(12); else setAndreyFontSize(28);
		 */
		...
		double[][] andreySize = { { moveTableWidth, TableLayout.FILL, 90 },
				{ 20, 40, andreySpace, 20, TableLayout.FILL, 30, 30, andreySpace, 40, 20 } };

		// JFrame framer = new JFrame("" + getBoardWidth() + " height " +
		// getBoardHeight() + " control length " + getControlLength() + " control height
		// " + getControlHeight());
		...
		/*
		 * add(andreyNavig, "0, 5, 2, 5"); andreyNavig.add(backwardEnd);
		 * andreyNavig.add(backward); andreyNavig.add(forward);
	...
	void makeLayout() {
		...
		if (isAndreyLayout() == false) {
			...
			ParallelGroup v1 = buttonLayout.createParallelGroup(GroupLayout.Alignment.LEADING);
			// was leading
		...
		ParallelGroup vb1 = buttonLayout.createParallelGroup(GroupLayout.Alignment.LEADING);
		// was leading
		...
		/*
		 * topClock = new JPanel(); topClock.setLayout(new GridLayout(2,1)); bottomClock
		 * = new JPanel(); bottomClock.setLayout(new GridLayout(2,1));
		 * 
		 * sliderArrows = new JPanel(); sliderArrows.setLayout(new GridLayout(3,1));
		 * sliderArrows.add(sharedVariables.moveSliders[gameData.BoardIndex]);
		 * sliderArrows.add(buttonPanel); sliderArrows.add(actionPanel);
		 */
		...
		/*
		 * add(topClock); add(gameListingDisplay);
		 * 
		 * add(listScroller);
		 * 
		 * add(sliderArrows);
