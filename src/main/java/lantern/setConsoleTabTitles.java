...
class setConsoleTabTitles {
	...
	void createConsoleTabTitle(channels sharedVariables, int conNumber, subframe[] consoleSubframes, String Name) {
		...
		/*
		 * if (title.length() > 7) { title=title.substring(0, 6); title=title + "."; }
		 */
		...
		for (int a = 0; a < sharedVariables.openConsoleCount; a++) {
			if (consoleSubframes[a] != null) {
				if (Name.length() > 0)
					consoleSubframes[a].channelTabs[conNumber]
							.setFullText(sharedVariables.consoleTabCustomTitles[conNumber]);
				else
				...
				if (sharedVariables.looking[a] == conNumber) {
					...
					consoleSubframes[a]
							.setTitle(consoleSubframes[a].consoleTitle + sharedVariables.consoleTabTitles[conNumber]);
				}
