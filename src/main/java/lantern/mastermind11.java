...
class mastermind11 extends JInternalFrame {
	...
	mastermind11() {
		...
		super("Mastermind", true, // resizable
				true, // closable
				true, // maximizable
				true);// iconifiable
			...
			try {
				...
				for (int a = mydata.guessMax - 1; a >= 0; a--) {
					...
					for (int b = 0; b < mydata.guessLimit; b++) {
						...
						g2.fill(new Rectangle2D.Double((double) originX + b * width,
								(double) originY + (mydata.guessMax - a - 1) * height, width - gap, height - gapY));
						...
						g2.fill(new Rectangle2D.Double(
								(double) (originX * 1.3) + mydata.guessLimit * width + b * width / 2,
								(double) originY + (mydata.guessMax - a - 1) * height, (width / 2) - gap,
								height - gapY));
					...
					if (totalScores == 0 && a < mydata.guessTop)
						g2.drawString("Nothing Scored", (int) (originX * 1.3 + mydata.guessLimit * width),
								(int) (originY + (mydata.guessMax - a - 1) * height + height / 2));
				} // end outer for
				if (mydata.state == mydata.DONE) {
					...
					for (int b = 0; b < mydata.guessLimit; b++) {
						...
						g2.fill(new Rectangle2D.Double((double) originX + b * width, (double) masterOriginY,
								width - gap, height - gapY));
					} // end for
					g2.setPaint(black);
					if (winner == true)
						g2.drawString("You win", (int) (originX + mydata.guessLimit * width + 20),
								(int) masterOriginY + 10);
					else
						g2.drawString("You lose", (int) (originX + mydata.guessLimit * width + 20),
								(int) masterOriginY + 10);
				...
				// now color tabs
				g2.setColor(red);
				g2.fill(new Rectangle2D.Double((double) colorTabsX[0], (double) colorTabsY[0], width - gap,
						height - gapY));

				g2.setPaint(blue);
				g2.fill(new Rectangle2D.Double((double) colorTabsX[1], (double) colorTabsY[1], width - gap,
						height - gapY));

				g2.setPaint(green);
				g2.fill(new Rectangle2D.Double((double) colorTabsX[2], (double) colorTabsY[2], width - gap,
						height - gapY));

				g2.setPaint(yellow);
				g2.fill(new Rectangle2D.Double((double) colorTabsX[3], (double) colorTabsY[3], width - gap,
						height - gapY));

				g2.setPaint(purple);
				g2.fill(new Rectangle2D.Double((double) colorTabsX[4], (double) colorTabsY[4], width - gap,
						height - gapY));

				g2.setPaint(orange);
				g2.fill(new Rectangle2D.Double((double) colorTabsX[5], (double) colorTabsY[5], width - gap,
						height - gapY));
...
// paint row of 6 colors to click for guesses
		...
		public void mousePressed(MouseEvent e) {
			...
			/*
			 * JFrame myframe = new JFrame(); myframe.setSize(100,100); myframe.setTitle(""
			 * + num); myframe.setVisible(true);
			 */
