...
// we are storing events in both a table and list
		...
		/*
		 * for(int ii=i; ii<200; ii++) model.add(ii, "" + ii);
		 */
	}// end constructor
	...
	void addToEvents(String entry, String number, String join, String watch, String info) {
		...
		if (join.equals("") && info.equals("")
				&& (watch.toLowerCase().startsWith("observe ") && !entry.startsWith("LIVE")))
		...
		for (int i = 0; i < model.size(); i++)
			if (modeldata.elementAt(i).equals(index)) {
				...
				try {
					...
					eventsTable.rowsRemoved(new TableModelEvent(eventsTable, i, i, TableModelEvent.ALL_COLUMNS,
							TableModelEvent.DELETE));
					topGamesTable.removeRow(i);
					topGamesTable.rowsRemoved(new TableModelEvent(eventsTable, i, i, TableModelEvent.ALL_COLUMNS,
							TableModelEvent.DELETE));
