...
class saveScores {
	...
	void saveNow(channels sharedVariables) {
		...
		for (a = 0; a < sharedVariables.mineScores.score9by9.top; a++)
			set_string = set_string + "///" + sharedVariables.mineScores.score9by9.list.get(a).score + "##"
					+ sharedVariables.mineScores.score9by9.list.get(a).name + "##"
					+ sharedVariables.mineScores.score9by9.list.get(a).date + "##::::";
		set_string = set_string + "[done9]::::";

		set_string = set_string + "[16]::::";
		for (a = 0; a < sharedVariables.mineScores.score16by16.top; a++)
			set_string = set_string + "///" + sharedVariables.mineScores.score16by16.list.get(a).score + "##"
					+ sharedVariables.mineScores.score16by16.list.get(a).name + "##"
					+ sharedVariables.mineScores.score16by16.list.get(a).date + "##::::";
		set_string = set_string + "[done16]::::";

		set_string = set_string + "[30]::::";
		for (a = 0; a < sharedVariables.mineScores.score16by30.top; a++)
			set_string = set_string + "///" + sharedVariables.mineScores.score16by30.list.get(a).score + "##"
					+ sharedVariables.mineScores.score16by30.list.get(a).name + "##"
					+ sharedVariables.mineScores.score16by30.list.get(a).date + "##::::";
		set_string = set_string + "[done30]::::";

		set_string = set_string + "[Custom]::::";
		for (a = 0; a < sharedVariables.mineScores.scoreCustom.top; a++)
			set_string = set_string + "///" + sharedVariables.mineScores.scoreCustom.list.get(a).score + "##"
					+ sharedVariables.mineScores.scoreCustom.list.get(a).name + "##"
					+ sharedVariables.mineScores.scoreCustom.list.get(a).date + "##::::";
	...
	void readNow(channels sharedVariables) {
			...
			for (ii = 0; ii < theDumpArray.length; ii++) {
				...
				if (line.startsWith("///")) {
					...
					if (n != -1) {
						...
						try {
							scoreTime = Integer.parseInt(time);
						} catch (Exception d) {
						}
		...
		/*
		 * JFrame frame = new JFrame(); JPanel mypanel = new JPanel();
		 * frame.add(mypanel); JLabel lab = new JLabel(dummy); mypanel.add(lab);
