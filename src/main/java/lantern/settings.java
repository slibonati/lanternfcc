...
// analysisBackgroundColor
		...
		/*
		 * to add onMoveBoardBackgroundColor responseColor defaultChannelColor kibcolor
		 * qtellcolor
		 * 
		 */
...
// notifysound
		...
		for (int boarc = 0; boarc < sharedVariables.maxConsoleTabs; boarc++) {
			...
			set_string = set_string + "" + sharedVariables.myConsoleSizes[boarc].point0.x + " "
					+ sharedVariables.myConsoleSizes[boarc].point0.y + " ";
			set_string = set_string + "" + sharedVariables.myConsoleSizes[boarc].con0x + " "
					+ sharedVariables.myConsoleSizes[boarc].con0y + " ";
			...
			for (int boar = 0; boar < myboards.length; boar++) {
				if (myboards[boar] != null)
					if (sharedVariables.myBoardSizes[boar].con0x > 10) {
						...
						set_string = set_string + "" + sharedVariables.myBoardSizes[boar].point0.x + " "
								+ sharedVariables.myBoardSizes[boar].point0.y + " ";
						set_string = set_string + "" + sharedVariables.myBoardSizes[boar].con0x + " "
								+ sharedVariables.myBoardSizes[boar].con0y + " ";
		...
		set_string = set_string + "" + sharedVariables.myActivitiesSizes.point0.x + " "
				+ sharedVariables.myActivitiesSizes.point0.y + " ";
		set_string = set_string + "" + sharedVariables.myActivitiesSizes.con0x + " "
...
// sshout console
		...
		for (int cona = 0; cona < sharedVariables.openConsoleCount; cona++) {
			for (int conb = 0; conb < sharedVariables.maxConsoleTabs; conb++) {
				...
				// scroll through these and set the selector on read settings. our consoles are
				// allready created
			} // end for
		...
		try {
			...
			while (tokens.hasMoreElements() && !temp.equals("")) {
				...
				if (temp.startsWith("[Chan")) {
					...
					try {
						int tnum = Integer.parseInt("" + temp.charAt(5));// this will break if console numbers become
																			// two digits or if they mess with ini file
																			// it would fail try
						int twoDigit = tnum;
				...
				if (temp.startsWith("[QChan")) {
					...
					try {
						int tnum = Integer.parseInt("" + temp.charAt(6));// this will break if console numbers become
																			// two digits or if they mess with ini file
																			// it would fail try
						int twoDigit = tnum;
...
// **********************************************************************************
				...
				if (temp.equals("[FonInput]")) {
					...
					try {
						...
						if (aFont != null) {
							...
							// we set the font after settings is called with the myconsolepanel.setfont()
							// methos outside this class/file
			...
			System.err.println("settings exception: " + e.getMessage());
		}
