...
//super(frame, mybool);
		/*
		 * super("Activities Window- double click to select", true, //resizable true,
		 * //closable true, //maximizable true);//iconifiable
		 */
...
//theChannelList.setBackground(sharedVariables.listColor);
	...
	void initComponents() {
		...
		/*
		 * try { displayList = new listClass(); add(displayList.theList);
		 * }catch(Exception d) { }
		 */
...
//list = new JList(data); //data has type Object[]
		...
		/* grab font */
		/*
		 * String aFont=theEventsList.getFont().getFontName(); aFont=aFont.replace(" ",
		 * "*");
		 * 
		 * 
		 * JFrame fontFrame = new JFrame("" + aFont + " " +
		 * theEventsList.getFont().getSize()); fontFrame.setSize(500,100);
...
//listScroller.setPreferredSize(new Dimension(2500, 2500));
		...
		MouseListener mouseListenerEvents = new MouseAdapter() {
			public void mouseClicked(MouseEvent e) {
				...
				/*
				 * JFrame framer = new JFrame("join is: " + join + " and watch is: " + watch +
				 * " and info is: " + info + " and col is: " + col); framer.setSize(1000,100);
				...
				if (join.equals("!!!") && info.equals("!!!")
						&& (watch.toLowerCase().startsWith("observe ") && !listing.startsWith("LIVE"))) {
				 ...
				 * framer.setVisible(true); framer.setSize(500,100);
				 */
			}// end method
		};// end class
...
//old list listener
		/*
		 * MouseListener mouseListenerEvents = new MouseAdapter() { public void
		 * mouseClicked(MouseEvent e) {
		 * 
		 * 
		 * if (e.getClickCount() == 2) { int index =
		 * theEventsList.locationToIndex(e.getPoint()); String listing =
		 * eventsList.getEventListing(index); String join =
		 * eventsList.getJoinCommand(index); String info =
		 * eventsList.getInfoCommand(index); String watch =
		 * eventsList.getWatchCommand(index);
		 * 
		 * boolean go=false; if(listing.equals("-")) return;
		 * if(listing.contains("[VIDEO]")) { if(!info.equals(""))
		 * if(info.startsWith("http://")) {
		 * 
		 * if(!join.equals("")) if(join.toLowerCase().contains(" webcast")) { go=true;
		 * sharedVariables.openUrl(info); } } if(!join.equals(""))
		 * if(join.startsWith("https://")) {
		 * 
		 * if(!join.equals("")) if(join.toLowerCase().contains("gotd")) { go=true;
		 * sharedVariables.openUrl(join); } } } if(join.equals("!!!") &&
		 * info.equals("!!!") && (watch.toLowerCase().startsWith("observe ") &&
		 * !listing.startsWith("LIVE"))) { myoutput data = new myoutput();
		 * data.consoleNumber = 0; data.data=watch + "\n"; queue.add(data); go=true;
		 * 
		 * } if(watch.equals("!!!") && info.equals("!!!") &&
		 * join.toLowerCase().startsWith("examine ")) { myoutput data = new myoutput();
		 * data.consoleNumber = 0; data.data=join + "\n"; queue.add(data); go=true;
		 * 
		 * } if(go == false) { eventDialog aDialog; aDialog= new eventDialog(homeFrame,
		 * false, listing, join, info, watch); aDialog.setSize(350,200);
		 * aDialog.setVisible(true); }// if go==false
		 * 
		 * 
		 * } } };
		 */
...
// set default visible
		...
		tournamentLabel.addMouseListener(new MouseAdapter() {
			...
			public void mouseReleased(MouseEvent e) {
				// turn on events and off seeks
				/*
				 * if(!listScroller.isVisible()) { seeklistScroller.setVisible(false);
				 * notifylistScroller.setVisible(false);
				 * computerseeklistScroller.setVisible(false); listScroller.setVisible(true);
		...
		seeksLabel.addMouseListener(new MouseAdapter() {
			...
			public void mouseReleased(MouseEvent e) {
				// turn on events and off seeks
				/*
				 * if(!listScroller.isVisible()) { listScroller.setVisible(false);
				 * notifylistScroller.setVisible(false);
				 * computerseeklistScroller.setVisible(false);
				 * seeklistScroller.setVisible(true); paintComponents(getGraphics()); repaint();
				 * 
				 * }
				 */
			}
		...
		notifyLabel.addMouseListener(new MouseAdapter() {
			...
			public void mouseReleased(MouseEvent e) {
				 ...
				 * computerseeklistScroller.setVisible(false);
				 * notifylistScroller.setVisible(true); paintComponents(getGraphics());
				 * repaint();
				 * 
				 * }
				 * 
		...
		computerSeeksLabel.addMouseListener(new MouseAdapter() {
			...
			public void mouseReleased(MouseEvent e) {
				...
				/*
				 * if(!listScroller.isVisible()) { listScroller.setVisible(false);
				 * seeklistScroller.setVisible(false); notifylistScroller.setVisible(false);
				 * computerseeklistScroller.setVisible(true); paintComponents(getGraphics());
				 * 
				 * }
				 */
			}
...
// seek display dialog
	...
	class eventDialog extends JDialog {
		...
		eventDialog(JFrame frame, boolean mybool, String event, final String join, final String info,
				final String watch) {
			super((JFrame) frame, mybool);
			...
			buttonjoin.addActionListener(new ActionListener() {
				public void actionPerformed(ActionEvent event) {
					 ...
					 * if(sharedVariables.myname.length() > 0 && sharedVariables.mypassword.length()
					 * > 0) { sharedVariables.openUrl(join1 + "?user=" + sharedVariables.myname +
					 * "&pass=" + sharedVariables.mypassword); return; }
					 */
	...
	class EventsPanel extends JPanel {
		EventsPanel() {
			...
			// layout.createParallelGroup(GroupLayout.Alignment.LEADING, true);// was
			// leading
			SequentialGroup vGroup = layout.createSequentialGroup();

			vGroup.addComponent(videoButton, GroupLayout.PREFERRED_SIZE, GroupLayout.DEFAULT_SIZE,
					GroupLayout.PREFERRED_SIZE);
			vGroup.addComponent(listScroller, GroupLayout.PREFERRED_SIZE, GroupLayout.DEFAULT_SIZE, Short.MAX_VALUE);
