...
class connectFour extends JInternalFrame implements ActionListener {
	...
	connectFour() {
		...
		super("Connect Four", true, // resizable
				true, // closable
				true, // maximizable
				true);// iconifiable
		...
		if (event.getActionCommand().equals("New Game")) {
			...
			if (parm.noDraw == 1) {
				...
				if (turntype == 0) {
					...
					mythreadobject runthis = new mythreadobject(maxX, maxY, xtops, parm, fourgame, myboard, xhit,
							gridXpoint, gridYpoint, gridHeight, gridWidth);
...
// end action performed method
	...
	class fourPanel extends JPanel implements MouseMotionListener, MouseListener {
		...
		public void paintComponent(Graphics g2) {
			...
			for (int a = 0; a < maxY + 1; a++)
				g.fill(new Rectangle2D.Double((double) (gridXpoint), (double) (gridYpoint + a * gridHeight),
						(double) (maxX * gridWidth), (double) 2));
			for (int a = 0; a < maxX + 1; a++)
				g.fill(new Rectangle2D.Double((double) (gridXpoint + a * gridWidth), (double) (gridYpoint), 2,
						(double) (maxY * gridHeight)));
			// draw filled squares
			g.setColor(white);
			for (int a = 0; a < maxX; a++)
				for (int b = 0; b < maxY; b++) {
					if (myboard[b * maxX + a] == 1) {
						g.setColor(red);
						g2.fillOval((int) (gridXpoint + 2 + a * gridWidth), (int) (gridYpoint + 2 + b * gridHeight),
								(int) (gridWidth - 2), (int) (gridHeight - 2));
					}
					...
					if (parm.lastx == a && parm.lasty == b) {
						...
						g.fill(new Rectangle2D.Double(
								(double) (gridXpoint + ((int) (gridWidth / 2) - 3) + a * gridWidth),
								(double) (gridYpoint + ((int) (gridHeight / 2) - 3) + b * gridHeight), 6, 6));
					}
		...
		public void mousePressed(MouseEvent e) {
			...
			if (aHit == 1 && xtops[xhit] < maxY && parm.turn % 2 == turntype) {
				yhit = maxY - (xtops[xhit]) - 1; // we no longer require they hit the y square just that there is an
													// available square in that column
				...
				repaint();
				/*
				 * if(parm.gameover == 0) { // set new game to disable //menuItem2.Enabled =
				 * false;
				 * 
				 * thread1.Start(); }
				 */
	...
	class mythreadobject implements Runnable {
		...
		public mythreadobject(int maxX1, int maxY1, int[] xtops1, myparameters parm1, fourclass fourgame1,
				int[] myboard1, int xhit1, int gridXpoint1, int gridYpoint1, int gridWidth1, int gridHeight1) {
			maxX = maxX1;
			maxY = maxY1;
