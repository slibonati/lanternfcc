...
public class channels {
	 ...
	 * http://www.pearlchess.com http://www.lanternchess.com for jars
	 * 
	 * 1) Lantern.java is renamed Pearl.java for fics // reverse for icc 2) Lantern
	 * class in this file is named Pearl 3) manifest.txt in same lantern folder has
	 * its main class changed to lantern.Pearl 4) fics is set to true // false for
	 * icc 5) free/a-fics/*.* files are coppied to free/a/*.* reverse
	 * free/a-chessclub/*.* for icc These files should be in a pearl.jar on
	 * pearlchess.com. This is due to my lack of understanding of how to set up the
	 * timestamp and timeseal jars properly not an intrinsic issue i think Anyone
	 * with this code who wants both fics and icc builds needs to just set up
	 * connect to properly use the timestamp or timseal jars in terms of file and
	 * jar placement in folder path structure.
	 * 
	 * notes - chessbot4.java is the main icc parser. chessbot4 routs parsing to
	 * DataParsing.java when fics is true. DataParsing.java is based on diamond
	 * chess for android parser
	 */
	static boolean macClient = true;
	/*
	 * maclient is true for all builds now and was originaly mac only true when
	 * implemented in 2019 for 6.22. It means the Public and Private directories set
	 * will be used on that OS. Documents/LanternChess can be a public directory for
	 * things a user would want to see like their log files. a private directory is
	 * in like Library on mac or appdata on windows then a LanternChess subfolder
	 * for things like settings On linux there just / currently which doesnt seem to
	 * produce a result different than macClient false in my test
	 */
...
//Container desktop;
	...
	long autoexamspeed;
	long lastSoundTime;// used to not send multiple sounds in say one second when forwarding through
						// games
	int lastSoundCount;
	...
	channels() {
		...
		try {
			/*
			 * observeIcon = new ImageIcon("images/observing.gif", "Observing"); wasIcon =
			 * new ImageIcon("images/was.gif", "Was"); playingIcon = new
			 * ImageIcon("images/playing.gif", "Playing"); examiningIcon = new
			 * ImageIcon("images/examining.gif", "Examining"); sposIcon = new
			 * ImageIcon("images/sposition.gif", "Sposition");
			 */
		...
		try {
			 ...
			 * dot.setSize(700,100); dot.setVisible(true);
			 */
		} catch (Exception efy) {
			chessclubIP = "207.99.83.228";
			chessclubPort = "5000";
			/*
			 * JFrame dot = new JFrame("failed. static ip"); dot.setSize(700,100);
			 * dot.setVisible(true);
			 */
		}
...
// now see if they got a bigger screen
		...
		kibStyle = 0;
		typedStyle = 1;
		consoleLayout = 1;// different layouts of subframe console. 1 is all tabs in row. 2 is two rows.
							// can make more
...
// current white/ blue board
	...
	String addHashTellWrapper(String mes, int number, boolean wrapTells) {
		...
		if (!wrapTells) {
			if (mes.toLowerCase().startsWith("tell ") || mes.toLowerCase().startsWith("tel ")
					|| mes.toLowerCase().startsWith("te ") || mes.toLowerCase().startsWith("t ")) {
				return mes;
			}
		}
		if (mes.toLowerCase().startsWith("shout ") || mes.toLowerCase().startsWith("shou ")
				|| mes.toLowerCase().startsWith("sho ") || mes.toLowerCase().startsWith("sh ")
				|| mes.toLowerCase().startsWith("i ")) {
			return mes;
		}

		if (mes.toLowerCase().startsWith("history ") || mes.toLowerCase().startsWith("histor ")
				|| mes.toLowerCase().startsWith("histo ") || mes.toLowerCase().startsWith("hist ")
				|| mes.toLowerCase().startsWith("his ") || mes.toLowerCase().startsWith("hi ")) {
			return mes;
		}
	...
	void addWallPaper() {
		try {
			...
			if (new File(wallpaperFileName).exists()) {
				...
				wallpaperImage = Toolkit.getDefaultToolkit().getImage(wallpaperURL);
	...
	void challengeCreator(String opponent, JFrame framer, ConcurrentLinkedQueue queue) {
		 ...
		 * 
		 * mychallenger.setLocation(px, py); } catch (Exception centerError) {}
		 */
		mychallenger.setLocation(framer.getLocation().x + framer.getSize().width / 2 - defaultWidth / 2,
				framer.getLocation().y + framer.getSize().height / 2 - defaultHeight / 2);
			...
			public void run() {
				...
				try {
					...
					if (os.indexOf("win") >= 0) {
						...
						// rt.exec( "open " + myurl);
						/*
						 * Class fileMgr = Class.forName("com.apple.eio.FileManager"); Method openURL =
						 * fileMgr.getDeclaredMethod("openURL", new Class[] {String.class});
						 * openURL.invoke(null, new Object[] {myurl});
						 * 
						 * http://www.java2s.com/Code/Java/Development-Class/LaunchBrowserinMacLinuxUnix
						 * .htm
						 */
						...
						for (int yy = 0; yy < channelNamesList.size(); yy++) {
							if (Integer.parseInt(channelNamesList.get(yy).channel) == Integer.parseInt(channel)) {
								...
								for (int y = 0; y < channelNamesList.get(chan).model2.size(); y++) {
									...
									if (channelNotifyList.get(z).nameList.get(x).toLowerCase()
											.equals(temp.toLowerCase())) {
										String realName = temp;
										boolean go = true;
										for (int m = 0; m < realnames.size(); m++)
											if (realnames.get(m).equals(temp))
