...
class createWindows {
	...
	createWindows(channels sharedVariables1, subframe[] consoleSubframes1, gameboard[] myboards1, JTextPane[] consoles1,
			...
			chatframe[] consoleChatframes1, Multiframe masterFrame1) {
	...
	protected void createConsoleFrame() {
		...
		// our consoles, main to sub console ( for channels) are allways created here.
		// the start of the program does a call to this method and our main console is
		// created. openConsoleCount is incremented from 0 to 1 and next console is
		// indexed at 1.
		if (sharedVariables.openConsoleCount >= sharedVariables.maxConsoleTabs - 1) // openConsolecount initializes at 0
																					// so main is always created at 0
																					// and first sub console at 1 up to
																					// 9 for a total of 10
		...
		if (sharedVariables.openConsoleCount == 1)// new trick now going to at startup turn on all 10 consoles but not
													// make the rest 2-10 visible
		{
			for (int z = sharedVariables.openConsoleCount; z < sharedVariables.maxConsoleTabs; z++) {
				consoleSubframes[sharedVariables.openConsoleCount] = new subframe(sharedVariables, consoles, queue,
						myDocWriter, myboards, this);
			 ...
			 * consoleChatframes[sharedVariables.maxConsoleTabs-2] = new
			 * chatframe(sharedVariables, consoles, queue, myDocWriter); //
			 ...
			 * consoleChatframes[sharedVariables.maxConsoleTabs-1] = new
			 * chatframe(sharedVariables, consoles, queue, myDocWriter); //
			 * consoleChatframes[sharedVariables.maxConsoleTabs-1].setVisible(true);
			 * sharedVariables.openConsoleCount++;
			 * 
			...
			public void run() {
				try {
					...
					if (!mywebframe.isVisible() || url.startsWith("<")) {
						...
						try {
							/*
							 * if(url.startsWith("<")) { consoles[0].setContentType("text/html");
							 * consoles[0].setText(url); } else
	...
	protected void restoreConsoleFrame() {
		...
		for (int a = 0; a < sharedVariables.maxConsoleTabs - 1; a++)// last console is detached
			if (consoleSubframes[a] != null) // we look for the first console ( and next if we loop further) that is not
												// null ( created)
				if (consoleSubframes[a].isVisible() == false) // we check if this console is visiable, if not we grab
																// its number in num to work with and break. note
																// restore happens in order of first console you can
																// restore and only one gets restored at a time.
				{
					num = a;
					break;
				}
		...
		sharedVariables.openConsoleCount = num; // bit of a trick. openConsoleCount is now == to that num we found ( or
												// invisiable console). were going to restore openConsoleCount to its
												// true number later.
		// the consoles increment open console count and take their number from it. we
		// are dubbing in a number and revering any changes ( incremening) they do
...
// patch routine to restore board to same size if its first  board
			...
			if (sharedVariables.myConsoleSizes[num].con0x != -1) {
				consoleSubframes[num].setSize(sharedVariables.myConsoleSizes[num].con0x,
						sharedVariables.myConsoleSizes[num].con0y);
				consoleSubframes[num].setLocation(sharedVariables.myConsoleSizes[num].point0.x,
						sharedVariables.myConsoleSizes[num].point0.y);
			} else
	...
	protected void createGameFrame() {
		...
		for (t1 = 0; t1 < sharedVariables.maxGameTabs; t1++) {
			if (myboards[t1] == null)
				break;
			else if (myboards[t1].isVisible() == false && (sharedVariables.mygame[t1].myGameNumber != -100
				...
				for (tab = 0; tab < sharedVariables.openBoardCount; tab++) {
					...
					myboards[t1].myconsolepanel.channelTabs[tab]
		...
		for (t1 = 0; t1 < sharedVariables.maxGameTabs; t1++) {
			if (myboards[t1] == null)
				...
				myboards[t1].setVisible(true);
				updateBoardsMenu(t1);
				sharedVariables.mygame[t1] = new gamestate(sharedVariables.excludedPiecesWhite,
				...
				for (int draw = 0; draw < sharedVariables.maxGameTabs; draw++)
					if (myboards[draw] != null)
						myboards[draw].myconsolepanel.channelTabs[mylast]
								.setText("" + sharedVariables.tabTitle[sharedVariables.tabLooking[mylast]], mylast);
					else
						break;
				sharedVariables.mygame[t1].imclosed = false;
		 ...
		 * if(myboards[d]!=null) if(!myboards[d].isVisible()) { boardNumber=d; break; }
		 * if(myboards[boardNumber]!=null) // && boardNumber !=
		 * sharedVariables.openBoardCount { if(sharedVariables.useTopGames == true) {
		 * if(myboards[boardNumber].topGame != null)
		 * myboards[boardNumber].topGame.setVisible(true); }
		 * 
		 * else myboards[boardNumber].setVisible(true);
		 * 
		 * boolean closed = true;
		 * 
		 * closed = false;
		 * 
		 * if(closed == true) { sharedVariables.openBoardCount++;
		 * sharedVariables.tabLooking[sharedVariables.openBoardCount - 1] = boardNumber;
		 * for(int cc=0; cc< sharedVariables.openBoardCount; cc++)
		 * if(myboards[cc]!=null) if(myboards[cc].isVisible() == true)
		 * myboards[cc].myconsolepanel.channelTabs[sharedVariables.openBoardCount-1].
		 * setVisible(true); int mylast = sharedVariables.openBoardCount-1;
		 * sharedVariables.tabTitle[sharedVariables.tabLooking[mylast]] = "G" + mylast;
		 * for(int draw=0; draw < sharedVariables.maxGameTabs; draw++)
		 * if(myboards[draw]!=null)
		 * myboards[draw].myconsolepanel.channelTabs[mylast].setText("" +
		 * sharedVariables.tabTitle[sharedVariables.tabLooking[mylast]], mylast); else
		 * break;
		 * 
		 * } else { JFrame haha = new JFrame("haha"); haha.setSize(200,200);
		...
		try {
			...
			for (tab = 0; tab < sharedVariables.maxGameTabs; tab++) {
				if (myboards[tab] != null) {
					...
					myboards[tab].myconsolepanel.channelTabs[boardNumber1]
							.setText(sharedVariables.tabTitle[sharedVariables.tabLooking[boardNumber1]], boardNumber1);
				} // end if not null
				else
					break;
			} // end for
			for (tab = 0; tab < sharedVariables.openBoardCount; tab++) {
				myboards[boardNumber].myconsolepanel.channelTabs[tab]
						.setText(sharedVariables.tabTitle[sharedVariables.tabLooking[tab]], tab);
			}
			...
			// add desktop to consolesubframe so it can call its method of focus traversal
			// between boards and consoles
...
// patch routine to restore board to same size if its first  board
			...
			if (sharedVariables.useTopGames == false) {
				...
				if (sharedVariables.myBoardSizes[boardNumber].con0x != -1) {
					myboards[boardNumber].setSize(sharedVariables.myBoardSizes[boardNumber].con0x,
							sharedVariables.myBoardSizes[boardNumber].con0y);
					myboards[boardNumber].setLocation(sharedVariables.myBoardSizes[boardNumber].point0.x,
							sharedVariables.myBoardSizes[boardNumber].point0.y);
				} else
					...
					public void run() {
						try {
							...
							if (sharedVariables.myBoardSizes[boardNumber1].con0x != -1) {
								myboards[boardNumber1].topGame.setSize(sharedVariables.myBoardSizes[boardNumber1].con0x,
										sharedVariables.myBoardSizes[boardNumber1].con0y);
								myboards[boardNumber1].topGame.setLocation(
										sharedVariables.myBoardSizes[boardNumber1].point0.x,
										sharedVariables.myBoardSizes[boardNumber1].point0.y);
							} else
								myboards[boardNumber1].topGame.setSize(sharedVariables.defaultBoardWide,
										sharedVariables.defaultBoardHigh);
						} catch (Exception e1) {
							// ignore
						}
					}
		...
		// JFrame framer = new JFrame("open board count is " +
		// sharedVariables.openBoardCount + " and board number is " + boardNumber);
		// framer.setSize(200,100);
		// framer.setVisible(true);

	}

	protected void createListFrame(listClass eventsList, listClass seeksList, listClass computerSeeksList,
			listClass notifyList, listClass tournamentList, JFrame homeFrame) {
		/*
		 * if(myfirstlist == null) { JFrame master = new JFrame(); myfirstlist = new
		 * listFrame(master, sharedVariables, queue, eventsList, seeksList,
		 * computerSeeksList, notifyList, homeFrame); }
		 */
		try {
...
//sharedVariables.desktop.add(myfirstlist);
		try { // myfirstlist.setSelected(true);
