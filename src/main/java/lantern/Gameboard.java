...
class Gameboard extends JInternalFrame implements InternalFrameListener, ComponentListener {
	...
	Gameboard(JTextPane consoles1[], subframe consoleSubframes1[], JTextPane gameconsoles1[],
		...
		try {
			...
			if (sharedVariables.mygame[gameData.BoardIndex] == null)
				sharedVariables.mygame[gameData.BoardIndex] = new gamestate(sharedVariables.excludedPiecesWhite,
						sharedVariables.excludedPiecesBlack, sharedVariables.excludedBoards);

			// writeout("going to create overall\n");
			myconsolepanel = new gameboardConsolePanel(topGame, consoles, consoleSubframes, sharedVariables, gameData,
					gameconsoles, gamequeue, queue, myDocWriter, mycontrolspanel, mypanel);
			...
			public void run() {
				try {
					if (top == true) {
						...
						myconsolepanel = new gameboardConsolePanel(topGame, consoles, consoleSubframes, sharedVariables,
								gameData, gameconsoles, gamequeue, queue, myDocWriter, mycontrolspanel, mypanel);
	...
	boolean getSidewayValue() {
		if ((sharedVariables.sideways == 1 || (sharedVariables.sideways == 0 && isMaximum()))
	...
	int getConsoleWidth() {
		if (getSidewayValue()) {
			return (int) (sharedVariables.boardConsoleSizes[3] * 1.8);
		}
		return (int) (sharedVariables.boardConsoleSizes[sharedVariables.boardConsoleType] * 1.8);
	}
	...
	class overallpanel extends JPanel {
		...
		void setOverallVertical() {
			...
			ParallelGroup vGroup = layout.createParallelGroup(GroupLayout.Alignment.LEADING, true);
			// was leading
		...
		if (tempnumber == sharedVariables.mygame[gameData.BoardIndex].myGameNumber) {
			...
			try {
				...
				for (int a = 0; a < sharedVariables.maxGameTabs; a++) {
					if (sharedVariables.gamelooking[a] == gameData.BoardIndex && sharedVariables.gamelooking[a] != -1
		...
		try {
			...
			if (i > -1) {
				...
				if (fen.substring(i + 1, i + 2).equals("b") && sharedVariables.mygame[gameData.BoardIndex].wild == 20
						&& sharedVariables.mygame[gameData.BoardIndex].movetop == 0) {
					moveSent(icsGameNumber, "e2e2", "-", false);
			...
			if (sharedVariables.mygame[gameData.BoardIndex].state != sharedVariables.STATE_OBSERVING
					&& !sharedVariables.mygame[gameData.BoardIndex].engineFen.equals("*")) {
	...
	void updateFicsBoard(String icsGameNumber, String boardLexigraphic, String styleline, String currentPlayer,
			String wShort, String wLong, String bShort, String bLong, String flipped, String doublePawnPushFile,
			...
			try {
				...
				if (sharedVariables.mygame[gameData.BoardIndex].state != sharedVariables.STATE_PLAYING) {
					sharedVariables.mygame[gameData.BoardIndex]
							.flipSent(sharedVariables.mygame[gameData.BoardIndex].board);
				} else {
	...
	void initialFicsInfo(String icsGameNumber, String rType, String r, String wElo, String bElo, String wTitle,
			String bTitle) {
		int tempnumber = getGameNumber(icsGameNumber);
		if (tempnumber == sharedVariables.mygame[gameData.BoardIndex].myGameNumber) {
			...
			sharedVariables.mygame[gameData.BoardIndex].name1 = sharedVariables.mygame[gameData.BoardIndex].realname1
					+ " " + wTitle + " " + wElo;
			sharedVariables.mygame[gameData.BoardIndex].name2 = sharedVariables.mygame[gameData.BoardIndex].realname2
					+ " " + bTitle + " " + bElo;
			repaint();
	...
	void newGameRelation(String icsGameNumber, String relation) {
		...
		if (tempnumber == sharedVariables.mygame[gameData.BoardIndex].myGameNumber) {
			if (relation.equals("E")) {
				...
				sharedVariables.mygame[gameData.BoardIndex].title = icsGameNumber + " Examining "
						+ sharedVariables.mygame[gameData.BoardIndex].realname1 + " vs "
						+ sharedVariables.mygame[gameData.BoardIndex].realname2;
	...
	void gameStarted(String icsGameNumber, String WN, String BN, String wildNumber, String rating_type, String rated,
			String white_initial, String white_inc, String type, String white_rating, String black_rating,
		...
		try {
			...
			setObservedPgnFile(timeCheck, Integer.parseInt(wildNumber));// called automaticly but only logs if observed
																		// and pgnObservedLogging on
		...
		if (rating_type.equals("Wild") || rating_type.startsWith("Loser")) {
			...
			sharedVariables.mygame[gameData.BoardIndex].gameListing = "" + white_initial + " " + white_inc + " "
					+ ratedDisplay + " " + wt.getWildNameString(wildNumber);
		} else
			sharedVariables.mygame[gameData.BoardIndex].gameListing = "" + white_initial + " " + white_inc + " "
					+ ratedDisplay + " " + rating_type;
		...
		try {
			...
			System.err.println("timer initialization exception: " + duty.getMessage());
		}
	...
	void logFicsGameToPGN() {
		try {
			...
			String game = "\r\n";
			/*
			 * [Event "ICC 5 0"] [Site "Internet Chess Club"] [Date "2011.10.09"] [Round
			 * "-"] [White "Mike"] [Black "Prophet-Daniel"] [Result "1-0"] [ICCResult
			 * "Black forfeits on time"] [WhiteElo "1046"] [BlackElo "943"] [Opening
			 * "Sicilian"] [ECO "B54"] [NIC "SI.01"] [Time "03:46:21"] [TimeControl "300+0"]
			 */
			String date = "*";
			String theTime = "*";
			try {
			...
			game += "[Event \"FICS " + wildString + sharedVariables.mygame[gameData.BoardIndex].time + " "
					+ sharedVariables.mygame[gameData.BoardIndex].inc + isRated + "\"]\r\n";
			...
			/*
			 * if(iccresult.equals("")) { iccresult = "*"; }
			 */
			game += "[Result \"" + sharedVariables.mygame[gameData.BoardIndex].ficsResult + "\"]\r\n";
			/*
			 * if(!iccresultstring.equals("")) { game += "[ICCResult \"" + iccresultstring +
			 * "\"]\r\n"; }
			 */
...
// year.month.day
			...
			game += "[Event \"ICC " + wildString + sharedVariables.mygame[gameData.BoardIndex].time + " "
					+ sharedVariables.mygame[gameData.BoardIndex].inc + isRated + "\"]\r\n";
...
// now moves
	...
	void updateCorrSposition() {
		...
		sharedVariables.mygame[gameData.BoardIndex].myGameNumber = sharedVariables.STATE_OVER;
	...
	void gameEnded(String icsGameNumber) {
		...
		if (tempnumber == sharedVariables.mygame[gameData.BoardIndex].myGameNumber) {
			...
			/*
			 * myoutput output = new myoutput(); output.tab=gameData.BoardIndex;
			 * output.tabTitle = "W" +
			 * sharedVariables.mygame[gameData.BoardIndex].myGameNumber; queue.add(output);
			 */
			if (sharedVariables.mygame[gameData.BoardIndex].state == sharedVariables.STATE_EXAMINING) {
			...
			sharedVariables.mygame[gameData.BoardIndex].title = "game over - "
			...
			for (int d = 0; d < sharedVariables.openBoardCount; d++)
				if (sharedVariables.mygametable[sharedVariables.gamelooking[d]] != null)
					if (sharedVariables.gamelooking[d] == gameData.BoardIndex) {
						...
						sharedVariables.mygametable[gameData.BoardIndex]
								.setChessFontForMoveList(sharedVariables.chessFontForMoveList);
						...
						sharedVariables.gametable[gameData.BoardIndex]
								.setModel(sharedVariables.mygametable[gameData.BoardIndex].gamedata);
					}
		} // end try
	...
	void gameEndedExamined(String icsGameNumber) {
		...
		if (tempnumber == sharedVariables.mygame[gameData.BoardIndex].myGameNumber) {
			...
			sharedVariables.mygame[gameData.BoardIndex].title = "game over now examined- "
					+ sharedVariables.mygame[gameData.BoardIndex].myGameNumber;
			...
			try {
				/*
				 * if(isVisible() == false) timer.cancel() ; //Terminate the thread // clocks
				 * were stopping with this code on new games. don't know why
				 * 
				 * // MA 12-10-10
				 */
			} catch (Exception bad) {
	...
	void updateWhiteName(String icsGameNumber, String value) {
		int tempnumber = getGameNumber(icsGameNumber);
		if (tempnumber == sharedVariables.mygame[gameData.BoardIndex].myGameNumber) {
			sharedVariables.mygame[gameData.BoardIndex].name1 = value + " "
					+ sharedVariables.mygame[gameData.BoardIndex].realelo1;
	...
	void updateBlackName(String icsGameNumber, String value) {
		int tempnumber = getGameNumber(icsGameNumber);
		if (tempnumber == sharedVariables.mygame[gameData.BoardIndex].myGameNumber) {
			sharedVariables.mygame[gameData.BoardIndex].name2 = value + " "
					+ sharedVariables.mygame[gameData.BoardIndex].realelo2;
	...
	void updateClock(String icsGameNumber, String colort, String time) {
		...
		try {
			...
			if (colort.equals("W")) {
				...
				sharedVariables.mygame[gameData.BoardIndex].whiteMinute = getMinutes(
						sharedVariables.mygame[gameData.BoardIndex].whiteClock);
				sharedVariables.mygame[gameData.BoardIndex].whiteSecond = getSeconds(
				...
				sharedVariables.mygame[gameData.BoardIndex].blackMinute = getMinutes(
						sharedVariables.mygame[gameData.BoardIndex].blackClock);
				sharedVariables.mygame[gameData.BoardIndex].blackSecond = getSeconds(
	...
	int getTenths(double ms) {
		/*
		 * int min = getMinutes(ms/1000); int sec = getSeconds(min, ms/1000); double dif
		 * = (min * 60 + sec) * 1000; dif = ms - dif; //milliseconds under a second int
		 * dif2 = (int) dif * 10; if(dif2 < 10 && dif2 > 0) return dif2;
		 */
		try {
			int sec = (int) (ms / 1000);
	...
	void repaintClocks() {
		/*
		 * Point topPoint = mycontrolspanel.topClockDisplay.getLocation(); Point
		 * botPoint = mycontrolspanel.botClockDisplay.getLocation(); Dimension
		 * topSize=mycontrolspanel.topClockDisplay.getSize(); Dimension
		 * botSize=mycontrolspanel.botClockDisplay.getSize();
		 * 
		 * mycontrolspanel.repaint(topPoint.x, topPoint.y, topSize.width,
		 * topSize.height); mycontrolspanel.repaint(botPoint.x, botPoint.y,
		...
		if (sharedVariables.mygame[gameData.LookingAt].wild == 24)// paint bug partners clock on gameboardpanel
			mypanel.repaint((int) mypanel.getWidth() / 2, 0, (int) mypanel.getWidth() / 2,
					sharedVariables.myGameFont.getSize() + 5);
	}
	...
	class ToDoTask extends TimerTask {
			...
			try {
				if (isVisible() == true)
					if (sharedVariables.mygame[gameData.LookingAt].myGameNumber != sharedVariables.NOT_FOUND_NUMBER
							&& sharedVariables.mygame[gameData.LookingAt].myGameNumber != sharedVariables.STATE_OVER)
		...
		public void run() {
			...
			if (sharedVariables.mygame[gameData.BoardIndex].myGameNumber == sharedVariables.NOT_FOUND_NUMBER
					|| sharedVariables.mygame[gameData.LookingAt].myGameNumber == sharedVariables.STATE_OVER) {
			...
			SwingUtilities.invokeLater(new Runnable() {
				public void run() {
					...
					if (((sharedVariables.mygame[gameData.BoardIndex].turn + 1) % 2 == 1
							&& sharedVariables.mygame[gameData.BoardIndex].wild != 30)
							|| ((sharedVariables.mygame[gameData.BoardIndex].turn + 1) % 2 == 0
									&& sharedVariables.mygame[gameData.BoardIndex].wild == 30)) {
						whiteOnMove = true;
					...
					if (whiteOnMove) {
						...
						try {
							...
							sharedVariables.mygame[gameData.BoardIndex].wtime = sharedVariables.mygame[gameData.BoardIndex].whiteClock;
							sharedVariables.mygame[gameData.BoardIndex].wtime = (double) sharedVariables.mygame[gameData.BoardIndex].wtime
									- time / 1000;
						...
						try {
							...
							sharedVariables.mygame[gameData.BoardIndex].btime = sharedVariables.mygame[gameData.BoardIndex].blackClock;
							sharedVariables.mygame[gameData.BoardIndex].btime = (double) sharedVariables.mygame[gameData.BoardIndex].btime
									- time / 1000;
	...
	class AutoExamTask extends TimerTask {
		...
		public void run() {
			...
			/*
			 * if(sharedVariables.autoexamspeed != myspeed) { autotimer.cancel(); autotimer
			 * = new Timer (); autotimer.scheduleAtFixedRate ( new AutoExamTask ( ) ,
	...
	void moveSent(String icsGameNumber, String amove, String algabraicMove, boolean makeSound) {
		...
		if (tempnumber == sharedVariables.mygame[gameData.BoardIndex].myGameNumber) {
			...
			/*
			 * try { if (!newMove.contains("@") &&
			 * sharedVariables.mygame[gameData.BoardIndex].wild!=20) newMove =
			 * pgnGetter.getPgn(amove, sharedVariables.mygame[gameData.BoardIndex].
			 * iflipped, sharedVariables.mygame[gameData.BoardIndex].boardCopy); }
			 * catch(Exception dy) {}
			 */
			...
			if (!sharedVariables.mygame[gameData.BoardIndex].premove.equals("")) {
				...
				if ((sharedVariables.mygame[gameData.BoardIndex].realname1.equals(sharedVariables.myname)
						&& movetop % 2 == 1)
						|| (sharedVariables.mygame[gameData.BoardIndex].realname2.equals(sharedVariables.myname)
								&& movetop % 2 == 0)) {
					makePremove();
			...
			for (int a = 0; a < sharedVariables.maxGameTabs; a++) {
				if (sharedVariables.gamelooking[a] == gameData.BoardIndex && sharedVariables.gamelooking[a] != -1
					...
					sharedVariables.moveSliders[a].setMaximum(sharedVariables.mygame[gameData.BoardIndex].turn + 1);
			...
			if (!(amove.contains("?") && sharedVariables.mygame[gameData.BoardIndex].wild == 16)) {
				...
				if (xfrom >= 0) // not @ drop
					type = sharedVariables.mygame[gameData.BoardIndex].makemove(xfrom + yfrom * 8, xto + yto * 8, prom,
							0, castleCapture, algabraicMove);
				// second to last field is reload
				else
					type = sharedVariables.mygame[gameData.BoardIndex].makemove(xfrom, xto + yto * 8, prom, 0,
							castleCapture, algabraicMove);
				// second to last field is reload
			} else {
				type = 0;
			...
			try {
				 ...
				 * (sharedVariables.mygame[gameData.BoardIndex].movetop, amove.substring(0,4));
				 * else sharedVariables.mygametable[gameData.BoardIndex].addMove
				 ...
				 * sharedVariables.mygame[gameData.BoardIndex].wild == 20 ) { if (amove.length()
				 * > 4) sharedVariables.mygametable[gameData.BoardIndex].addMove
				 * (sharedVariables.mygame[gameData.BoardIndex].movetop, amove.substring(0,4));
				 * else sharedVariables.mygametable[gameData.BoardIndex].addMove
				 * (sharedVariables.mygame[gameData.BoardIndex].movetop, amove);
				 * 
				 ...
				 */
				sharedVariables.mygametable[gameData.BoardIndex]
						.addMove(sharedVariables.mygame[gameData.BoardIndex].movetop, newMove);
				...
				try {
					for (int aa = 0; aa < sharedVariables.maxGameTabs; aa++) {
						...
						if (sharedVariables.gamelooking[aa] == gameData.BoardIndex
								&& sharedVariables.gamelooking[aa] != -1 && sharedVariables.gametable[aa] != null) {
							final int aaa = aa;
							SwingUtilities.invokeLater(new Runnable() {
								@Override
								public void run() {
									try {
										if (sharedVariables.moveSliders[aaa]
												.getValue() == sharedVariables.moveSliders[aaa].getMaximum())
											sharedVariables.gametable[aaa]
													.scrollRectToVisible(sharedVariables.gametable[aaa].getCellRect(
				...
				if (sharedVariables.makeSounds == true && makeSound == true)
					if (sharedVariables.mygame[gameData.BoardIndex].state == sharedVariables.STATE_PLAYING
						...
						if (sharedVariables.lastSoundTime + 200 > System.currentTimeMillis()
								&& sharedVariables.lastSoundCount > 1)
	...
	void illegalMove(String icsGameNumber, String moveString, String reason) {
		...
		if (tempnumber == sharedVariables.mygame[gameData.BoardIndex].myGameNumber) {
			...
			if (sharedVariables.mygame[gameData.BoardIndex].wild == 30 && reason.equals("11")) // checkers illegal move
																								// means they have
																								// another move to jump
				return;
	...
	void sendUciMoves() {
		...
		if (!Channels.fics) {
			...
			if (sharedVariables.mygame[gameData.BoardIndex].engineFen.length() > 2) {
				...
				moves = "";
				if (!sharedVariables.mygame[gameData.BoardIndex].engineFen
						.startsWith("rnbqkbnr/pppppppp/8/8/8/8/PPPPPPPP/RNBQKBNR"))
					moves = "ucinewgame\n";
				moves += "position fen " + sharedVariables.mygame[gameData.BoardIndex].engineFen
			...
			outgoing.data += "stop\nucinewgame\nposition fen "
					+ sharedVariables.mygame[gameData.BoardIndex].getStockfishFen();
	...
	void Backward(String icsGameNumber, String count) {
		...
		if (tempnumber == sharedVariables.mygame[gameData.BoardIndex].myGameNumber) {
			...
			for (int a = 0; a < sharedVariables.maxGameTabs; a++) {
				if (sharedVariables.Looking[a] == gameData.BoardIndex && sharedVariables.Looking[a] != -1
						&& sharedVariables.moveSliders[a] != null) {
					sharedVariables.moveSliders[a].setMaximum(sharedVariables.mygame[gameData.BoardIndex].turn);
			...
			try {
				...
				sharedVariables.mygametable[gameData.BoardIndex]
						.removeMoves(sharedVariables.mygame[gameData.BoardIndex].movetop + num, num);
			 ...
			 * 
			 * } catch (Exception e1) { //ignore } } }); }//end try catch(Exception dumb) {}
			 */
			sharedVariables.mygame[gameData.BoardIndex].replay();
			if (sharedVariables.mygame[gameData.BoardIndex].state == sharedVariables.STATE_EXAMINING) {
				sharedVariables.mygame[gameData.BoardIndex].computeHash();
	...
	/*
	 * public void windowClosing(WindowEvent e) { // we want to serialize the window
	 * dimensions
	 * 
	 * if (isVisible() && isMaximum() == false && isIcon() == false) {
	 * setBoardSize(); }
	 ...
	 * myconsolepanel.myself.switchConsoleWindows();
	 * 
	 * setVisible(false); if (sharedVariables.mygame[gameData.LookingAt].state !=
	 * sharedVariables.STATE_PLAYING) { myoutput data = new myoutput();
	 * data.closetab=gameData.LookingAt; queue.add(data); } }
	 * 
	 ...
	 * }
	 * 
	 * public void windowDeiconified(WindowEvent e) { if(isVisible() && isMaximum()
	 * == false && isIcon() == false) { setBoardSize(); } }
	 * 
	 * public void windowActivated(WindowEvent e) { if (isVisible() && isMaximum()
	 * == false && isIcon() == false) { setBoardSize(); } giveFocus(); }
	 * 
	 ...
	 * }
	 * 
	 * boolean isMaximum() { return false; }
	 * 
	 * boolean isIcon() { return false; }
	 * 
	 * void setMaximum(boolean home) { return; }
	 */
...
class randomPieces {
	...
	void randomizeGraphics() {
		...
		blackPieceNum = getChoiceBlack(randomGenerator.nextInt(getMaxPieceChoiceBlack() - 1), whitePieceNum, maxDepth);

		// boardNum = randomGenerator.nextInt(temp.maxBoards);
		boardNum = getChoiceBoard(randomGenerator.nextInt(getMaxBoardChoice()), boardNum, maxDepth);
	}
	...
	int getChoiceBlack(int num, int otherset, int depth) {
		...
		if (depth > 0)
			return getChoiceBlack(randomGenerator.nextInt(getMaxPieceChoiceBlack() - 1), otherset, depth - 1);
...
class TimeDisplayClass {
	...
	String getWhiteTimeDisplay(int Looking) {
		...
		if (sharedVariables.showTenths == 1) {
			...
			if (sharedVariables.mygame[Looking].whiteMinute == 0 && wsecI < 15
		...
		if (sharedVariables.mygame[Looking].whiteSecond >= 0 && sharedVariables.mygame[Looking].whiteMinute >= 0
				&& sharedVariables.mygame[Looking].wtime >= 0) {
			if (sharedVariables.mygame[Looking].whiteSecond < 10 && sharedVariables.mygame[Looking].whiteSecond > -10)
				text = " " + wmin + ":0" + wsec;
			else
				text = " " + wmin + ":" + wsec;
		} else {
	...
	String getBlackTimeDisplay(int Looking) {
		...
		if (sharedVariables.showTenths == 1) {
			...
			if (sharedVariables.mygame[Looking].blackMinute == 0 && bsecI < 15
		...
		if (sharedVariables.mygame[Looking].blackSecond >= 0 && sharedVariables.mygame[Looking].blackMinute >= 0
				&& sharedVariables.mygame[Looking].btime >= 0) {

			if (sharedVariables.mygame[Looking].blackSecond < 10 && sharedVariables.mygame[Looking].blackSecond > -10)
				text = " " + bmin + ":0" + bsec;
			else
				text = " " + bmin + ":" + bsec;
