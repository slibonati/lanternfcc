...
class seekPanel extends JPanel implements MouseMotionListener, MouseListener {
	...
	void setDimensions() {
		...
		/*
		 * if(bulletSeekW > seekHeight) bulletSeekW = seekHeight; if(blitzSeekW >
		 * seekHeight) blitzSeekW = seekHeight; if(standardSeekW > seekHeight)
		 * standardSeekW = seekHeight;
		 * 
		 * 
		 * if(seekHeight > bulletSeekW && seekHeight > blitzSeekW && seekHeight >
		 * standardSeekW) { if(bulletSeekW > blitzSeekW && bulletSeekW > standardSeekW)
		 * seekHeight = bulletSeekW; else if(blitzSeekW > bulletSeekW && blitzSeekW >
		 * standardSeekW) seekHeight = blitzSeekW; else seekHeight = standardSeekW;
		 * 
		 * }
		 */
	}
...
//g2.fill(new Rectangle2D.Double((double) standardBaseX,  0, 2, (double) baseHeightBottom));
			...
			g2.drawString(" 1000", 0,
					baseHeightBottom - seekGraphData.getHeightAt(1000) * seekHeight + (seekTextFont.getSize() / 2));
			g2.drawString(" 1500", 0,
					baseHeightBottom - seekGraphData.getHeightAt(1500) * seekHeight + seekTextFont.getSize() / 2);
			g2.drawString(" 2000", 0,
					baseHeightBottom - seekGraphData.getHeightAt(2000) * seekHeight + seekTextFont.getSize() / 2);
...
// draw bullet seeks
			...
			for (a = 0; a < sharedVariables.graphData.height * sharedVariables.graphData.bulletW; a++) {
				if (sharedVariables.graphData.bulletGrid[a] != null) {
					...
					if (y < 0)
						y = 0;
					drawSeek(g2, sharedVariables.graphData.bulletGrid[a].col,
							sharedVariables.graphData.bulletGrid[a].compCol, bulletBaseX, x, y, bulletSeekW, seekHeight,
							sharedVariables.graphData.bulletGrid[a].rated,
							sharedVariables.graphData.bulletGrid[a].computer,
							sharedVariables.graphData.bulletGrid[a].onNotify);
				}
			} // end draw bullet
...
// draw blitz seeks
			...
			for (a = 0; a < sharedVariables.graphData.height * sharedVariables.graphData.blitzW; a++) {
				if (sharedVariables.graphData.blitzGrid[a] != null) {
					...
					if (y < 0)
						y = 0;
					drawSeek(g2, sharedVariables.graphData.blitzGrid[a].col,
							sharedVariables.graphData.blitzGrid[a].compCol, blitzBaseX, x, y, blitzSeekW, seekHeight,
							sharedVariables.graphData.blitzGrid[a].rated,
							sharedVariables.graphData.blitzGrid[a].computer,
							sharedVariables.graphData.blitzGrid[a].onNotify);
				}
...
// draw standard seeks
			...
			for (a = 0; a < sharedVariables.graphData.height * sharedVariables.graphData.standardW; a++) {
				if (sharedVariables.graphData.standardGrid[a] != null) {
					...
					if (y < 0)
						y = 0;
					drawSeek(g2, sharedVariables.graphData.standardGrid[a].col,
							sharedVariables.graphData.standardGrid[a].compCol, standardBaseX, x, y, standardSeekW,
							seekHeight, sharedVariables.graphData.standardGrid[a].rated,
							sharedVariables.graphData.standardGrid[a].computer,
							sharedVariables.graphData.standardGrid[a].onNotify);
				}
			} // end draw standard
	...
	void drawSeek(Graphics2D g2, Color col, Color compColor, int originX, int x, int y, int width, int height,
			String rated, boolean computer, boolean onNotify) {
		...
		try {
			...
			/*
			 * if(onNotify==true) { if(col!=null && compColor!=null) drawSeek2(g2,
			 * col.darker().darker(), compColor.darker().darker(), originX, x, y, width,
			 * height, rated, computer, onNotify); else if(col!=null) drawSeek2(g2,
			 * col.darker().darker(), compColor, originX, x, y, width, height, rated,
			 * computer, onNotify); else if(compColor!=null) drawSeek2(g2, col,
			 * compColor.darker().darker(), originX, x, y, width, height, rated, computer,
			 * onNotify); else drawSeek2(g2, col, compColor, originX, x, y, width, height,
			 * rated, computer, onNotify); } else
			 */
	...
	void drawSeek2(Graphics2D g2, Color col, Color compColor, int originX, int x, int y, int width, int height,
			String rated, boolean computer, boolean onNotify) {
...
// x y width height
			if (compColor == null) {
				if (computer == true)
					g2.fill(new Rectangle2D.Double(originX + (x * width), (double) baseHeightBottom - y * height,
							(double) drawWidth - 1, (double) height - 1));
				else
					g2.fill(new Ellipse2D.Double(originX + (x * width), (double) baseHeightBottom - y * height,
							(double) drawWidth - 1, (double) height - 1));
			} else // 2 halves
			{
				g2.fill(new Rectangle2D.Double(originX + (x * width), (double) baseHeightBottom - y * height,
						(double) drawWidth - 1, (double) height / 2));
				g2.setColor(compColor);
				g2.fill(new Rectangle2D.Double(originX + (x * width),
						(double) baseHeightBottom - y * height + height / 2, (double) drawWidth - 1,
						(double) height / 2 - 1));
			...
			if (rated.equals("u") && compColor == null) {
				...
				if (computer == true)
					g2.fill(new Rectangle2D.Double(originX + (x * width) + 2,
							(double) baseHeightBottom - y * height + 2, (double) drawWidth - 5, (double) height - 5));
				else
					g2.fill(new Ellipse2D.Double(originX + (x * width) + 2, (double) baseHeightBottom - y * height + 2,
							(double) drawWidth - 5, (double) height - 5));
			} else if (rated.equals("u") && compColor != null) {
				g2.setColor(backColor);// background
				g2.fill(new Rectangle2D.Double(originX + (x * width) + 2, (double) baseHeightBottom - y * height + 2,
						(double) drawWidth - 5, (double) height - 6));
			}
	...
	void eventOutput(String eventDescription, MouseEvent e) {
		...
		if (over != null) {
			/*
			 * JFrame fr = new JFrame(); fr.setSize(150, 50); fr.setVisible(true);
