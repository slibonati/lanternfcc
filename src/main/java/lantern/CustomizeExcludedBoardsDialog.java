			...
			try {
				...
				for (int a = 0; a < 6; a++) {
					if (Boards == 0) {
						...
						g2.fill(new Rectangle2D.Double((double) boardx + a * square + dif, (double) boardy + dif,
								(double) square - dif, (double) square - 4));

					} // end solid colored board
					else {
						g.drawImage(graphics.boards[Boards][(a + 1) % 2], boardx + a * square + dif, boardy + dif,
								square - dif, square - 4, this);
					}
				}
