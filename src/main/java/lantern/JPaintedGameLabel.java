...
public class JPaintedGameLabel extends JLabel {
	...
	protected void paintBorder(Graphics g) {
		try {
			...
			g2.draw(new Line2D.Double(0, (double) height - 1, (double) width - 1, (double) height - 1)); // across
																											// bottom
			g2.draw(new Line2D.Double(0, 0, 0, (double) height)); // down
			g2.draw(new Line2D.Double((double) width - 1, 0, (double) width - 1, (double) height - 1)); // down right
																										// side
		} catch (Exception e) {
		}
	...
	public void setText(String s, int num) {
		...
		try {
			...
			/*
			 * if(num < 0) // 0-4 tabs get icons // now < 0 not < 5 so disabled the setting
			 * or using of icons on game tabs now { if(s.startsWith("O"))
			 * setIcon(sharedVariables.observeIcon); if(s.startsWith("W"))
			 * setIcon(sharedVariables.wasIcon); if(s.startsWith("P"))
			 * setIcon(sharedVariables.playingIcon); if(s.startsWith("E"))
			 * setIcon(sharedVariables.examiningIcon); if(s.startsWith("S"))
			 * setIcon(sharedVariables.sposIcon); if(s.startsWith("G"))
