...
class pgnWriter {
	...
	String getPgn(String moves, int iflipped, int[] board2) {
		...
		// for now write pgn as piece from to. i.e. Nb1c3, move generators need to check
		// uniqueness for shorter moves. exception pawn and king moves can be simplified
		...
		for (i = 0; i < theMoveArray.length; i++) {
			...
			try {
				...
				int xfrom = cordinator.getxmove(tryMove, 0, iflipped, 0); // move toggle iflipped turn with turn ok to
																			// always be 0 if not crazyhouse
				...
				int xto = cordinator.getxmove(tryMove, 2, iflipped, 0); // move toggle iflipped turn with turn ok to
																		// always be 0 if not crazyhouse
				...
				} else // a knight bishop or queen
				{
					...
					if (board[from] == 2 || board[from] == 8) {
						...
						top = generator.generateKnightMoves(fromList, toList, board, top, color, board[from]);// the 0
																												// is
																												// top ,
																												// this
																												// is
					...
					if (board[from] == 3 || board[from] == 9) {
						...
						top = generator.generateBishopMoves(fromList, toList, board, top, color, board[from]);// the 0
																												// is
																												// top ,
																												// this
																												// is
					...
					if (board[from] == 4 || board[from] == 10) {
						...
						top = generator.generateRookMoves(fromList, toList, board, top, color, board[from]);// the 0 is
																											// top ,
																											// this is
																											// first
																											// call
