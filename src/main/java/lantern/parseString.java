...
// we track the absolute characters indexes of each part of the tell
	...
	int getHyperIndex(String s) {
		 ...
		 * 
		 * if(index > -1) { int j=s.indexOf("\"", index + 1); int k=s.indexOf(" ", index
		 * + 1);
		 * 
		 * int go=0; if(j < k || (k==-1 && j !=-1)) { if(j!=-1) go=1; } if(go==1) return
		 * index; }
		 * 
		 */

		/*
		 * index=s.indexOf("\"observe"); if(index > -1) { int j=s.indexOf("\"", index +
		 * 1); int k=s.indexOf(" ", index + 1);
		 * 
		 * int go=0; if(j > k && k!=-1) { if(k+1 < s.length()) {int m=s.indexOf(" ",
		 * k+1); if(m>j || m == -1) go=1; } else go=1;
		 * 
		 * } if(go==1) { spaceLink=1; return index;
		 * 
		 * } }
		 * 
		 * index=s.indexOf("'observe");
