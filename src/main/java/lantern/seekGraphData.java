...
class seekGraphData {
	...
	void addSeek(String sIndex, String sName, String sTitles, String sRating, String sProvisional, String sWild,
			String sRatingType, String sTime, String sInc, String sRated, String sRange, String sColor, String sFormula,
			String sManual, listClass snotifyList) {
		seekInfo newInfo = new seekInfo(sIndex, sName, sTitles, sRating, sProvisional, sWild, sRatingType, sTime, sInc,
				sRated, sRange, sColor, sFormula, sManual, snotifyList);
	...
	int getSpot(int x, int y, int type, int rating) {
		...
		if (type == 0) // bullet
		{
			...
			if (bulletGrid[spot] != null) {
				boolean go = false;
				try {
					occupiedRating = Integer.parseInt(bulletGrid[spot].rating);
		...
		if (type == 1) // blitz
		{
			...
			if (blitzGrid[spot] != null) {
				boolean go = false;
				try {
					occupiedRating = Integer.parseInt(blitzGrid[spot].rating);
				} catch (Exception duii) {
				}
				if (occupiedRating < rating)
		...
		if (type == 2) // standard
		{
			...
			if (standardGrid[spot] != null) {
				boolean go = false;
				try {
					occupiedRating = Integer.parseInt(standardGrid[spot].rating);
				} catch (Exception duii) {
				}
				if (occupiedRating < rating)
