...
class minesweeper10 extends JInternalFrame implements ActionListener, InternalFrameListener {
	...
	public void actionPerformed(ActionEvent event) {
		...
		if (event.getActionCommand().equals("High Scores 9 by 9")) {
			highScoreDialog dialog = new highScoreDialog(masterFrame, true, sharedVariables.mineScores.score9by9,
					"High Scores 9 by 9");
		}

		if (event.getActionCommand().equals("High Scores 16 by 16")) {
			highScoreDialog dialog = new highScoreDialog(masterFrame, true, sharedVariables.mineScores.score16by16,
					"High Scores 16 by 16");
		}

		if (event.getActionCommand().equals("High Scores 16 by 30")) {
			highScoreDialog dialog = new highScoreDialog(masterFrame, true, sharedVariables.mineScores.score16by30,
					"High Scores 16 by 30");
		}
	...
	class minesweeperpanel extends JPanel implements MouseMotionListener, MouseListener {
					...
					for (int x = 1; x <= myGridData.collumns; x++) {
						if (myGridData.grid[x - 1][y - 1] == myGridData.UNCOVERED)
							...
							if (myGridData.state == myGridData.ONGOING
									&& myGridData.markedGrid[x - 1][y - 1] == myGridData.MARKED)
							...
							g2.setColor(new Color(0, 255, 2500));

						for (int a = 2; a < dimension - 2; a++)
							g2.draw(new Line2D.Double(originX + x * dimension + 2, originY + y * dimension + a,
									originX + (x + 1) * dimension - 2, originY + (y) * dimension + a));

						if (myGridData.grid[x - 1][y - 1] == myGridData.UNCOVERED
								&& myGridData.numbers[x - 1][y - 1] > 0) {
							g2.setColor(colorNumber);
							Font myFont = new Font("TimesRoman", Font.BOLD, 20);
							g2.setFont(myFont);

							g2.drawString("" + myGridData.numbers[x - 1][y - 1], originX + x * dimension + 7,
									originY + (y) * dimension + (int) (5 / 4 * dimension) - 5);
		...
		int getSquareXY(String type) {
			try {
				...
				/*
				 * if(myGridData.grid[x][y] == myGridData.BOMB) return BOMB;
				 */
				if (type.equals("X"))
					return x;
