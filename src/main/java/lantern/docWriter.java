...
class docWriter {
	...
	void processLink2(StyledDocument doc, String thetell, Color col, int index, int attempt, int game,
			SimpleAttributeSet attrs, int[] allTabs2, messageStyles myStyles) {
		allTabs = allTabs2;
	...
	void processLink3(StyledDocument doc, String thetell, int start, int stop, Color col, int index, int attempt,
			int game, SimpleAttributeSet attrs, messageStyles myStyles) {
		...
		try {
			/*
			 * Font font; if(game==SUBFRAME_CONSOLES) { font=
			 * sharedVariables.tabStuff[index].tabFont; if(font == null)
			 * font=sharedVariables.myFont; } else font=sharedVariables.myFont;
			 * 
			 * int x = consoleSubframes[0].getFontMetrics(font).stringWidth("asdf");
			 */

			/**************
			 * new djlogan code ********************* if(attempt == 0 && game == 0) { try {
			...
			if (mine.itsLink == 0) {
				...
				if (mine.text1.length() != 0) {
					if (attempt == 1) {
						...
						processLink3(doc, mine.text1, mine.start1, mine.stop1, col, index, attempt - 1, game, attrs,
								myStyles);
				}
...
//attrs.addAttribute(ID, new Integer(++linkID));
				...
				if (mine.text3.length() != 0) {
					...
					if (attempt == 1) {
						...
						processLink3(doc, mine.text3, mine.start3, mine.stop3, col, index, attempt - 1, game, attrs,
								myStyles);
			...
			allTabs = null; // could be on if they came in on process link 2 now its always off after
							// business is done
	...
	/*
	 * void writeToConsole(StyledDocument doc, int i) { return; } void
	 * writeToGameConsole(StyledDocument doc, int i) { return; }
	 * 
	 */
			...
			if (myboards[a] != null)
				if (myboards[a].isVisible()) {
...

					{
						...
						boolean go = true;
						if ((sharedVariables.mygame[i].state == sharedVariables.STATE_EXAMINING
								|| sharedVariables.mygame[i].state == sharedVariables.STATE_OBSERVING)
...
// 	insertWithStyles(doc, thetell, attrs, myStyles, start, stop, false);
	...
	void insertWithStyles(StyledDocument doc, String thetell, SimpleAttributeSet attrs, messageStyles myStyles,
			int start, int stop, boolean underline) {
		try {
			if (underline == true)
				StyleConstants.setUnderline(attrs, true);
// loop
...
// dasher feature make stuff between ` ` italic
					...
					if (italic1 > -1 && italic2 > -1
							&& !attrs.containsAttribute(StyleConstants.CharacterConstants.Italic, Boolean.TRUE)) {
						int dd = 0;
						while (dd != -1) {
...
//patchedInsertString(doc, doc.getLength(), myStrings[a].substring(dd, italic1), attrs);
								...
								patchedInsertString(doc, doc.getLength(), myStrings[a].substring(italic1, italic2 + 1),
										attrs);
								if (sharedVariables.italicsBehavior == 1)
								...
								dd = italic2 + 1;
								/*
								 * JFrame fram = new JFrame("italic1 " + italic1 + " and italic2 " + italic2 +
								 * " " + myStrings[a]); fram.setVisible(true); fram.setSize(300,300);
								 */
							} else {
								// insert end
								patchedInsertString(doc, doc.getLength(),
										myStrings[a].substring(dd, myStrings[a].length()), attrs);
