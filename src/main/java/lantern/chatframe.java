...
//JDesktopPaneCustom myself = (JDesktopPaneCustom) getDesktopPane();
		// customizeTabConsolelColorsDialog frame = new
		// customizeTabConsolelColorsDialog( this, false, sharedVariables, consoles,
		// con, this);
			...
			channelTabs[con].addMouseListener(new MouseAdapter() {
				...
				public void makerightclickhappen(MouseEvent e, final int n) {
					...
					item7.addActionListener(new ActionListener() {
						public void actionPerformed(ActionEvent e) {
							if (sharedVariables.tellsToTab == true && sharedVariables.tellTab == n) // i am curerntly
																									// directing tells
																									// to this tab
							{
		...
		consoles[consoleNumber].addMouseListener(new MouseAdapter() {
			public void mousePressed(MouseEvent e) {
				...
				try {
					...
					if (e.getButton() == MouseEvent.BUTTON3
							|| (e.getClickCount() == 2 && sharedVariables.autopopup == true)) {
...
//String underline="false";
						...
						/*
						 * Element elm=editor.getStyledDocument().getParagraphElement(pos); Document
						 * doc=elm.getDocument(); String doctext=""; try { doctext=doc.getText(0,
						 * doc.getLength()); } catch(Exception ee) {} JFrame aframe = new JFrame();
						 * JPanel panel = new JPanel(); JTextArea TA = new JTextArea();
						 * TA.setText(underline); TA.setColumns(40); TA.setRows(20);
						 * 
						 * panel.add(TA); aframe.add(panel); aframe.pack();
						 * 
						 * //aframe.setSize(100,100); aframe.setVisible(true);
						 * 
						 */
					}
				}
		...
		scrollbutton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent event) {
				 ...
				 * getVisibleRect(), SwingConstants.VERTICAL, -1 ); int
				 * f=sharedVariables.ConsoleScrollPane[consoleNumber].getVerticalScrollBar().
				 * getValue(); int g=
				 * sharedVariables.ConsoleScrollPane[consoleNumber].getVerticalScrollBar().
				 * getMaximum(); aframe.setTitle(" curernt value" + f + " and maximum " + g +
		...
		sharedVariables.ConsoleScrollPane[consoleNumber].getVerticalScrollBar()
				.addAdjustmentListener(new AdjustmentListener() {
					public void adjustmentValueChanged(AdjustmentEvent e) {
						...
						if (scrollnow == 1 && false == sharedVariables.ConsoleScrollPane[consoleNumber]
								.getVerticalScrollBar().getValueIsAdjusting()) {
							e.getAdjustable().setValue(e.getAdjustable().getMaximum());

						} // end if not adjusting
						else {
							if (true == sharedVariables.ConsoleScrollPane[consoleNumber].getVerticalScrollBar()
									.getValueIsAdjusting())
								scrollnow = 0;
							...
							try {
								...
								if (scrollnow == 1 && sharedVariables.ConsoleScrollPane[consoleNumber]
										.getVerticalScrollBar().getValueIsAdjusting() == false)
									e.getAdjustable().setValue(e.getAdjustable().getMaximum());
...
//updateSize();
	 ...
	 * 
	 * 
	 * } public void mouseEntered (MouseEvent me) {} public void mouseReleased
	 * (MouseEvent me) { if(me.isPopupTrigger())
	 * menu.show(me.getComponent(),me.getX(),me.getY()); }
	 * 
	 * public void mouseExited (MouseEvent me) {} public void mouseClicked
	 * (MouseEvent me) {
	 * 
	 * }
	 * 
	...
	void updateComboBox(int n) {
		try {
			...
			// first loop is to check items we would add against combo memory BEFORE we
			// remove the items triggering updates on combo memory
			for (a = 0; a < 400; a++)
				if (sharedVariables.console[sharedVariables.looking[consoleNumber]][a] == 1
					...
					if (aItem.equals(comboMemory[n])) {
						...
						foundIndex = count;// the idea is we want the index that we added the item that should be
											// selected ( its in comboMemory)
...
// now check names
			for (a = 0; a < sharedVariables.comboNames[sharedVariables.looking[consoleNumber]].size(); a++) {
				...
				if (aItem.equals(comboMemory[n])) {
					...
					foundIndex = count;// the idea is we want the index that we added the item that should be selected
										// ( its in comboMemory)
			...
			Input.addKeyListener(new KeyListener() {
				public void keyPressed(KeyEvent e) {
					...
					if (e.getModifiersEx() == 512)// alt
					{
						...
						if (a == 38) // alt + up arrow
						{
							...
							if (sharedVariables.tabStuff[con].tabFont != null) {
								...
								fontsize++;
								sharedVariables.tabStuff[con].tabFont = sharedVariables.tabStuff[con].tabFont
										.deriveFont(fontsize);
								makeHappen(con);
								return;
							} else {
					...
					if (a == 10) {
						...
						if (index > 0 && !Input.getText().startsWith("/")) // we done use tell channel prefix if starts
																			// with /
							mes = pre + mes;

						myoutput output = new myoutput();
						if (sharedVariables.myServer.equals("ICC") && sharedVariables.myname.length() > 0)
							output.data = "`c" + sharedVariables.looking[consoleNumber] + "`" + mes;// having a name
																									// means level 1 is
																									// on if on icc and
																									// this `phrase`mess
																									// will be used to
						...
						try {
							...
							if (sharedVariables.password == 0) {
								...
								if (sharedVariables.tabStuff[sharedVariables.looking[consoleNumber]].typed == true) {
									...
									int maxLinks = 75;
									myDocWriter.processLink(doc, mes, sharedVariables.typedColor,
											sharedVariables.looking[consoleNumber], maxLinks, SUBFRAME_CONSOLES, attrs,
											null);
									// doc.insertString(doc.getLength(), mes, attrs);
...
// code here
		...
		void setMyLayout1() {
			...
			h2.addComponent(sharedVariables.ConsoleScrollPane[consoleNumber], GroupLayout.Alignment.LEADING,
