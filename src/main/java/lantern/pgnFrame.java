...
//super(frame, mybool);
		super(myLoader1.title, true, // resizable
				true, // closable
				true, // maximizable
				true);// iconifiable
...
//list = new JList(data); //data has type Object[]
		...
		add(mypane);
		/*
		 * try { listScroller.setPreferredSize(new Dimension(2500, 2500)); }
		 * catch(Exception dd){}
		 */
			...
			void examine(int row) {
				...
				new Thread(new Runnable() {
					...
					public void run() {
						...
						for (int a = 0; a < myLoader.games.get(row).moves.size() - 1; a++)// size - 1 since last thing
